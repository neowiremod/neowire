...
local CompileVisitors = {
	...
	[NodeVariant.Block] = function(self, trace, data)
		...
		elseif self.scope:ResolveData("function") then
			-- If inside a function, check if returned.
			return function(state)
				---@param state RuntimeContext
				state.prf = state.prf + cost
				if state.prf > TickQuota then
			...
			self:Scope(function(scope)
				if ifeif[1] then -- if or elseif
					...
					if expr_ty == "n" then -- Optimization: Don't need to run operator_is on number (since we already check if ~= 0 here.)
						chain[i] = { expr, self:CompileStmt(ifeif[2]) }
					else
	...
	[NodeVariant.For] = function(self, trace, data)
		...
		local block = self:Scope(function(scope)
			scope.data.loop = true
			scope:DeclVar(var.value, {
				initialized = true,
				type = "n",
				trace_if_unused = var.trace,
			})
	...
	[NodeVariant.Foreach] = function(self, trace, data)
		...
		local block, cost = self:Scope(function(scope)
			scope.data.loop = true
			scope:DeclVar(key.value, {
				initialized = true,
				trace_if_unused = key.trace,
				type = key_type,
			})

			scope:DeclVar(value.value, {
				initialized = true,
				trace_if_unused = value.trace,
				type = value_type,
			})
			...
			for i = 1, ncases do
				...
				if case[1](state, expr) ~= 0 then
					...
					if state.__break__ then
						...
						goto exit
					elseif state.__return__ then
						-- Yes this should only be checked if the switch is inside a function, but I don't care enough about the performance of switch case to add another duplicated 30 lines to the file
						goto exit
	...
	[NodeVariant.Try] = function(self, trace, data)
		...
		self:Scope(function(scope)
			scope:DeclVar(err_var.value, {
				initialized = true,
				trace_if_unused = err_var.trace,
				type = "s",
			})
	...
	[NodeVariant.Function] = function(self, trace, data)
			...
			for i, param in ipairs(data[4]) do
				...
				elseif param.variadic then
					...
					self:Warning("Use of implicit parameter type is deprecated (add :number)", param.name.trace, {
						{
							replace = param.name.value .. ":number",
							at = param.name.trace,
						},
					})
				end
		...
		if variadic_ty then
			...
			if variadic_ty == "r" then
				function fn.op(state, args) ---@param state RuntimeContext
					...
					local scope = {
						vclk = {}, -- Hack in the fact that functions don't have upvalues right now.
					}
		...
		self:IsolatedScope(function(scope)
			for i, type in ipairs(param_types) do
				scope:DeclVar(param_names[i], {
					type = type,
					trace_if_unused = data[4][i] and data[4][i].name.trace or trace,
					initialized = true,
				})
			end
	...
	[NodeVariant.Include] = function(self, trace, data)
		...
		return function(state)
			...
			local s_scopes, s_scopeid, s_scope = state.Scopes, state.ScopeID, state.Scope
			local scope = {
				vclk = {}, -- Isolated scope, except global variables are shared.
			}
	...
	[NodeVariant.Assignment] = function(self, trace, data)
		...
		if data[1] then
			...
			self.scope:DeclVar(var_name, {
				initialized = true,
				trace_if_unused = data[2][1][1].trace,
				type = value_ty,
			})
			return function(state)
				---@param state RuntimeContext
		...
		for i, v in ipairs(data[2]) do
			...
			if existing then
				...
				self.global_scope:DeclVar(var, {
					type = value_ty,
					initialized = true,
					trace_if_unused = trace,
				})
	...
	[NodeVariant.Const] = function(self, trace, data)
		...
		self.scope:DeclVar(name, {
			type = expr_ty,
			initialized = true,
			const = true,
			trace_if_unused = data[1].trace,
		})
		return function(state)
	...
	[NodeVariant.CompoundArithmetic] = function(self, trace, data)
		...
		local result = Node.new(NodeVariant.ExprArithmetic, { Node.new(NodeVariant.ExprIdent, data[1], data[1].trace), data[2], data[3] }, trace)
	...
	[NodeVariant.ExprConstant] = function(self, trace, data, used_as_stmt)
		...
		elseif ty == "Vector" and wire_expression2_funcs["vec(nnn)"] then
			return self:CompileExpr(
				Node.new(
					NodeVariant.ExprCall,
					{
						Token.new(TokenVariant.String, "vec"),
						{ Node.new(NodeVariant.ExprLiteral, { "n", value[1] }, trace), Node.new(NodeVariant.ExprLiteral, { "n", value[2] }, trace), Node.new(NodeVariant.ExprLiteral, { "n", value[3] }, trace) },
					},
					trace
				)
			)
		...
		elseif ty == "table" then
			-- Know it's an array already from registerConstant
			local out = {}
			for i, val in ipairs(value) do
				...
				elseif ty == "Vector" then
					out[i] = Node.new(
						...
						{
							Token.new(TokenVariant.String, "vec"),
							{ Node.new(NodeVariant.ExprLiteral, { "n", val[1] }, trace), Node.new(NodeVariant.ExprLiteral, { "n", val[2] }, trace), Node.new(NodeVariant.ExprLiteral, { "n", val[3] }, trace) },
						},
						trace
					)
	...
	[NodeVariant.ExprFunction] = function(self, trace, data)
		---@type EnvFunction
		local fn, param_names, param_types, nargs = {
			attrs = {},
		}, {}, {}, #data[1]
		...
		local block = self:Scope(function(scope)
			...
			for i, param in ipairs(data[1]) do
				...
				scope:DeclVar(param.name.value, {
					type = param_types[i],
					initialized = true,
					trace_if_unused = param.name.trace,
				})
			end
	...
	[NodeVariant.ExprUnaryOp] = function(self, trace, data)
		...
		elseif data[1] == Operator.Sub then
			-- Negate
	...
	[NodeVariant.ExprCall] = function(self, trace, data, used_as_stmt)
		...
		if name.value == "changed" and data[2][1].variant == NodeVariant.ExprIdent and self.inputs[3][data[2][1].data.value] then
			self:Warning("Use ~ instead of changed() for inputs", trace, {
				{
					replace = "~" .. data[2][1].data.value,
					at = trace,
				},
			})
		end
		...
		elseif fn_data.attrs["legacy"] then
			-- Not a user function. Can get function to call at compile time.
			local fn, largs = fn_data.op, {
				[1] = {},
				[nargs + 2] = types,
			}
						...
						for i = nargs, 0, -1 do
							...
							if fn then
								...
								if fn.attributes.legacy then
									...
									return fn[3](state, largs, arg_types)
								elseif varsig == "array(...)" then
									-- Need this since can't enforce compile time argument type restrictions on string calls. Woop. Array creation should not be a function..
									local i = 1
	...
	[NodeVariant.Event] = function(self, trace, data)
		...
		for i, param in ipairs(data[2]) do
			...
			if not type then
				self:Warning("Use of implicit parameter type is deprecated", param.name.trace, {
					{
						replace = param.name.value .. ":number",
						at = param.name.trace,
					},
		...
		local block = self:IsolatedScope(function(scope)
			for k, arg in ipairs(event.args) do
				scope:DeclVar(params[k][1], {
					type = arg.type,
					initialized = true,
					trace_if_unused = params[k][3],
				})
			end
			return self:CompileStmt(data[3])
		end)

		self.registered_events[name][self.include or "__main__"] = function(state, args)
			---@param state RuntimeContext
			local s_scopes, s_scopeid, s_scope = state.Scopes, state.ScopeID, state.Scope
			local scope = {
				vclk = {}, -- Hack in the fact that functions don't have upvalues right now.
...
function Compiler:CompileExpr(node) --[[@param node Node]] ---@return RuntimeOperator, string
	...
	---@cast ty -nil # LuaLS can't figure this out yet.
	return op, ty
end
...
function Compiler:Process(ast)
	for var, type in pairs(self.persist[3]) do
		self.global_scope:DeclVar(var, {
			initialized = false,
			trace_if_unused = self.persist[5][var],
			type = type,
		})
	end
	...
	for var, type in pairs(self.inputs[3]) do
		self.global_scope:DeclVar(var, {
			initialized = true,
			trace_if_unused = self.inputs[5][var],
			type = type,
		})
	end
