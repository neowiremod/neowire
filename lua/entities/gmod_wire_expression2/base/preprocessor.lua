...
function PreProcessor:Error(message, trace, quick_fix)
	self.errors[#self.errors + 1] = Error.new(message, trace or Trace.new(self.readline, 1, self.readline, 1), nil, quick_fix)
end
...
function PreProcessor:Warning(message, trace, quick_fix)
	self.warnings[#self.warnings + 1] = Warning.new(message, trace or Trace.new(self.readline, 1, self.readline, 1), quick_fix)
end
...
function PreProcessor:GetType(tp, trace)
	...
	local up = tp:upper()
	if tp == "normal" then
		self:Warning("Use of deprecated type [normal]", trace, {
			{
				at = trace,
				replace = "number",
			},
	...
	repeat
		...
		if found then -- We found something
			...
			if (isinput or isoutput) and char:match("[A-Z]") ~= nil then -- we found the start of an input/output variable definition
				...
				count = count + 1
				ret[count] = {
					type = isinput and "inputs" or "outputs",
					name = varname,
					pos = found,
					blockcomment = {},
				}
...
function PreProcessor:RemoveComments(line)
	...
	for i = 1, num do
		...
		elseif not self.multilinestring then
			-- Else it's a comment if we're not inside a multiline string
...
local directive_handlers = {
	...
	["autoupdate"] = function(self)
		...
		if not IsValid(self.ent) or not self.ent.duped or not self.ent.filepath or self.ent.filepath == "" then
			return ""
		end
...
function PreProcessor:ParseDirectives(line)
	...
	if directive:lower() ~= directive then
		...
		self:Error("Directive (@" .. E2Lib.limitString(directive, 10) .. ") must be lowercase", tr, {
			{
				at = tr,
				replace = string.lower(directive),
			},
		})
	end
...
function PreProcessor:Process(buffer, directives, ent)
	...
	if not directives then
		self.directives = {
			...
			model = nil,
			inputs = {
				{}, -- 1: names, 2: types, 3: names=types lookup, 4: descriptions, 5: names={line, column} lookup
				{},
				{},
				{},
				{},
	...
	if not self.directives.name then
	...
	for column, key in ports:gmatch("()(%S+)") do
		...
		if i then
			...
			if not i then
				...
				self:Error("Variable name (" .. E2Lib.limitString(key, 10) .. ") must start with an uppercase letter", tr, {
					{
						at = tr,
						replace = key:sub(1, 1):upper() .. key:sub(2),
					},
				})
				...
				for column2, var in namestring:gmatch("()([^,]+)") do
					...
					if var ~= "" then
						...
						if not var:match("^[A-Z]") then
							self:Error("Variable name (" .. E2Lib.limitString(var, 10) .. ") must start with an uppercase letter", tr, {
								{
									at = tr,
									replace = var:sub(1, 1):upper() .. var:sub(2),
								},
							})
		...
		if character == ":" then
			...
			if vtype ~= vtype:lower() then
				self:Error("Variable type [" .. E2Lib.limitString(vtype, 10) .. "] must be lowercase", tr, {
					{
						at = tr,
						replace = vtype:lower(),
					},
			...
			elseif vtype == "normal" then
				self:Warning("Variable type [normal] is deprecated", tr, {
					{
						at = tr,
						replace = "number",
					},
...
function PreProcessor:PP_else(args, trace)
	...
	if state == nil then
		self:Error("Found #else outside #ifdef/#ifndef block", trace)
	end
	if args:Trim() ~= "" then
		self:Error("Must not pass an argument to #else", trace)
	end
	if self:Disabled() then
		table.insert(self.ifdefStack, false)
	else
...
function PreProcessor:PP_endif(args, trace)
	local state = table.remove(self.ifdefStack)
	if state == nil then
		self:Error("Found #endif outside #ifdef/#ifndef block", trace)
	end
	if args:Trim() ~= "" then
		self:Error("Must not pass an argument to #endif", trace)
	end
end
