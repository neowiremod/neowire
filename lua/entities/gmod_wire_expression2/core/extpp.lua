...
local RemovedOperators = {
	["operator!"] = true, -- These now use operator_is and operator== internally
	["operator!="] = true,
	["operator&"] = true, -- Despite && being binary AND in E2, the preprocessor used to handle this as logical AND.
	["operator&&"] = true,
	["operator|"] = true, -- Despite || being binary OR in E2, the preprocessor used to handle this as logical OR.
	["operator||"] = true,
	["operator++"] = true, -- Now use + and - internally.
	["operator--"] = true,
	["operator[]"] = true, -- indexget and indexset now.
	["operator[T]"] = true,
...
local function getTypeId(ty)
	...
	return
		-- Already registered.
		-- Preparsed from registerType
		(wire_expression_types[upper] and wire_expression_types[upper][1])
			or preparsed_types[upper]
...
local function parseParameters(raw, trace)
	...
	for k, raw_param in ipairs(split) do
		...
		elseif raw_param:match("^%s*%.%.%.%s*$") then
			-- Variadic lua parameter
...
local function parseAttributes(attributes, trace)
	...
	if attributes ~= "" and attributes:sub(1, 1) == "[" and attributes:sub(-1, -1) == "]" then
		local attrs = {
			legacy = "false", -- extpp can generate functions abiding by the new compiler.
		}
...
function E2Lib.ExtPP.Pass2(contents, filename)
	...
	local changed = false
	for a_begin, attributes, h_begin, ret, thistype, colon, name, args, whitespace, equals, h_end in
		contents:gmatch('()(%[?[%w,_ ="]*%]?)[\r\n\t ]*()e2function%s+(' .. p_typename .. ")%s+([a-z0-9]-)%s*(:?)%s*(" .. p_func_operator .. ")%(([^)]*)%)(%s*)(=?)%s*()")
	do
		...
		elseif RemovedOperators[name] then
			-- Old operator that no longer is needed.
			...
			elseif aliasflag == 2 then
				-- Override information with alias information.
			...
			if aliasflag then -- Add single newline, since aliasing only does anything in the footer.
				...
				elseif is_operator then
					-- Operators are directly passed the arguments, since they're known at compile time.
					table.insert(
						output,
						compact([[
	...
	-- for contents:gmatch(e2function)
