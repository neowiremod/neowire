...
local printcolor_readers = {
	[1] = function()
		return tostring(net.ReadDouble())
	end,
	[2] = function()
	...
	if console then
		...
		if not_warned and ply ~= LocalPlayer() then
			...
			if cvar_warn:GetBool() then
				chat.AddText(
					RED,
					"While in somone's seat/car/whatever, printColorDriver can be used to 100% realistically fake people talking, including admins.\
