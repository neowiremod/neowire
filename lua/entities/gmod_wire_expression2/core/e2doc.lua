if SERVER then
	...
	local function e2doc()
		...
		local function getTypeImage(typeid)
			...
			if typeimages[typeid] then
				return typeimages[typeid]
			end
		...
		timer.Simple(0.2, function()
			-- there's a timer in e2descriptions which we need to wait for
			...
			for signature, funcdata in pairs(wire_expression2_funcs) do
				...
				if string.sub(signature, 1, 3) == "op:" then
					continue
				end
				-- get file path
			...
			for filepath, data in pairs(files) do
				...
				if wireextras[filepath] then
					is_wire_extras = " (Wire Extras)"
				end
			...
			table.sort(nav, function(a, b)
				...
				if not a.iscustom and b.iscustom then
					return true
				end
	...
	concommand.Add("e2doc", function(player, command, args)
		if not file.IsDir("e2doc", "DATA") then
			file.CreateDir("e2doc")
		end
