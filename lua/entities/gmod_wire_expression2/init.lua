...
function ENT:UpdatePerf()
	...
	if self.context.prfcount < 0 then
...
function ENT:Execute()
	if self.error or not self.context or self.context.resetting then
		return
	end
...
function ENT:CompileCode(buffer, files, filepath)
	...
	if not status then
		return self:Error(directives[1].message)
	end
	self.buffer = buffer
	self.error = false
...
function ENT:ResetContext()
	...
	self.lastResetOrError = CurTime()
	local context = E2Lib.RuntimeContext.builder():withChip(self):withOwner(self.player):withStrict(self.directives.strict):withUserFunctions(self.funcs):withIncludes(self.includes)
	if self.context then
		context = context:withPrf(self.context.prf * resetPrfMult, self.context.prfcount * resetPrfMult, self.context.prfbench * resetPrfMult):withTime(self.context.time * resetPrfMult, self.context.timebench * resetPrfMult)
	end
...
function MakeWireExpression2(player, Pos, Ang, model, buffer, name, inputs, outputs, vars, inc_files, filepath, codeAuthor)
	if not player then -- For Garry's Map Saver
		player = game.GetWorld()
	end

	if IsValid(player) and not player:CheckLimit("wire_expressions") then
		return false
	end
	if not WireLib.CanModel(player, model) then
		return false
	end
...
--------------------------------------------------
local average_ram = 0
local enable = CreateConVar("wire_expression2_ram_emergency_shutdown_enable", "0", { FCVAR_ARCHIVE }, "Enable/disable the emergency shutdown feature.")
local average_halt_multiplier = CreateConVar("wire_expression2_ram_emergency_shutdown_spike", "4", { FCVAR_ARCHIVE }, "if (current_ram > average_ram * spike_convar) then shut down all E2s")
local halt_max_amount = CreateConVar("wire_expression2_ram_emergency_shutdown_total", "512", { FCVAR_ARCHIVE }, "This is in kilobytes, if (current_ram > total_convar) then shut down all E2s")
	...
	if enable:GetBool() then
		hook.Add("Think", "wire_expression2_emergency_shutdown", function()
			...
			if average_ram == 0 then -- set up initial value
				...
				if current_ram > average_ram * average_halt_multiplier:GetFloat() or current_ram > halt_max_amount:GetInt() * 1000 then -- if the current ram spikes -- or if the current ram goes over a set limit
