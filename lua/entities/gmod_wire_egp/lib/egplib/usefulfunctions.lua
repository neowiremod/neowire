...
-- Table IsEmpty
----------------------------
function EGP:Table_IsEmpty(tbl)
	return next(tbl) == nil
end
...
function EGP:SetScale(ent, x, y)
	if not self:ValidEGP(ent) or not x or not y then
		return
	end
	ent.xScale = { x[1], x[2] }
	ent.yScale = { y[1], y[2] }
...
function EGP:ScaleObject(ent, v)
	if not self:ValidEGP(ent) then
		return
	end
	local xScale = ent.xScale
	local yScale = ent.yScale
	if not xScale or not yScale then
		return
	end
	local xMin = xScale[1]
	local xMax = xScale[2]
...
function EGP:IsDifferent(tbl1, tbl2)
	if self:Table_IsEmpty(tbl1) ~= self:Table_IsEmpty(tbl2) then -- One is empty, the other is not
		return true
	end
...
function EGP:SendColor(obj)
	...
	if obj.a then
		net.WriteUInt(math.Clamp(obj.a, 0, 255), 8)
	end
end
	...
	hook.Add("PlayerInitialSpawn", "EGP_ScrHW_Request", function(ply)
		timer.Simple(1, function()
			if ply and ply:IsValid() and ply:IsPlayer() then
				umsg.Start("EGP_ScrWH_Request", ply)
				umsg.End()
			end
		end)
	end)
	...
	if size == 1 then -- size 1 => just normal lines
		...
		if vertices.outline_cache_size ~= size then -- check if the outline was cached already
			...
			if closed then
				...
				lastdir = {
					x = (x2 - x1) / len, -- initialize lastdir so first segment can be drawn normally
					y = (y2 - y1) / len,
				}
			end
					...
					if x1 ~= x2 or y1 ~= y2 then -- cannot get direction between identical points, just skip it
						if not closed and i == 1 then -- very first segment, just start perpendicular (TODO: maybe move before the loop)
							...
							elseif dot < -0.999 then
								-- new direction is inverse, just add perpendicular nodes
								corners[#corners + 1] = {
									r = {
										x = x1 - dir.y * size,
										y = y1 + dir.x * size,
							...
							elseif dir.x * -lastdir.y + dir.y * lastdir.x > 0 then
								-- right bend, checked by getting the dot product between dir and lastDir:rotate(90)
								...
								if dot < 0 then -- sharp corner, add two points to the outer edge to not have insanely long spikes
									corners[#corners + 1] = {
										...
										l = {
											x = x1 + (lastdir.x + lastdir.y) * size,
											y = y1 + (lastdir.y - lastdir.x) * size,
										},
									}
									...
									corners[#corners + 1] = {
										...
										l = {
											x = x1 - (dir.x - dir.y) * size,
											y = y1 - (dir.y + dir.x) * size,
										},
									}
								else
									corners[#corners + 1] = {
...
function EGP:EGPCursor(this, ply)
	...
	if B >= 0 then
		if monitor.Emitter then
			...
			if x < 0 or x > 512 or y < 0 or y > 512 then -- Aiming off the screen
				return ReturnFailure(this)
			end
...
function EGP.WorldToLocal(object, x, y)
	...
	if theta ~= 0 then
		...
		x, y = x * cos_theta - y * sin_theta, y * cos_theta + x * sin_theta
	end
	return x, y
end
...
function EGP.Draw(ent)
	...
	for _, obj in ipairs(rt) do
		if obj.parent == -1 or obj.NeedsConstantUpdate then
			ent.NeedsUpdate = true
		end
		if obj.parent ~= 0 then
			if not obj.IsParented then
				EGP:SetParent(ent, obj, obj.parent)
			end
