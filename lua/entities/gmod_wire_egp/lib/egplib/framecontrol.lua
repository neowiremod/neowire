...
function EGP:SaveFrame(ply, Ent, index)
	if not EGP.Frames[ply] then
		EGP.Frames[ply] = {}
	end
	if not EGP.FrameCounts[ply] then
		EGP.FrameCounts[ply] = 0
	end
	if EGP.FrameCounts[ply] > 256 and not EGP.Frames[ply][index] then -- TODO convar to change limit, 256 seems enough for an obscure feature
		return false
	end

	if not EGP.Frames[ply][index] then
		EGP.FrameCounts[ply] = EGP.FrameCounts[ply] + 1
	end
