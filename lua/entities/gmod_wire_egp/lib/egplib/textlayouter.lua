...
function TextWrapIndex:JustifyLine(width, startI, endI)
	if not self:GetJustify() then
		return
	end
	-- TODO: figure out why this needs to be here
	if not self.TextData[startI] or not self.TextData[endI] then
		return
	end
...
function TextWrapIndex:ComputeParagraph(line, y, numWords)
	...
	for word in line:gmatch("[^ ]+") do
		...
		splitWord, x, y, lineWidth, lastI, numWords = self:SplitWord(word, width, x, y, lineWidth, lastI, numWords)
