...
local function makeArray(v, fakepos)
	...
	if isstring(v.verticesindex) then
		if not fakepos then
			if not v["_" .. v.verticesindex] then
	...
	if bool then
		if parentindex == -1 then -- Parent to cursor?
			...
			if bool then
				EGP:AddParentIndexes(v)
				if SERVER then
					parentindex = math.Clamp(parentindex, 1, self.ConVars.MaxObjects:GetInt())
				end
				...
				if v.parent and v.parent == v.index then
					return false
				end
				-- If the user is trying to create a circle of parents, causing an infinite loop
				if not CheckParents(Ent, v, parentindex, {}) then
					return false
				end
				if v:Set("parent", parentindex) then
					return true, v
				end
			end
		end
...
function EGP:UnParent(Ent, index)
	...
	if bool then
		...
		if not v.parent or v.parent == 0 then
			return false
		end
		data.parent = 0
		if v:EditObject(data) then
			return true, v
		end
	end
end
