		...
		if LastItem.Ent == Ent and LastItem.Action == "Object" then
			for k, v in ipairs(LastItem.Args[1]) do
				if v.index == Object.index then
					...
					elseif v.ID ~= Object.ID then
						-- Not the same kind of object, create new
						if v.OnRemove then
							v:OnRemove()
						end
