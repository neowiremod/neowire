...
if SERVER then
	...
	function EGP:CheckInterval(ply)
		if not self.IntervalCheck[ply] then
			self.IntervalCheck[ply] = {
				bytes = 0,
				time = 0,
			}
		end
	...
	local function ClearScreen(Ent, ply)
		...
		if not EGP.umsg.Start("EGP_Transmit_Data", ply) then
			return
		end
		net.WriteEntity(Ent)
	...
	local function SendObjects(Ent, ply, DataToSend)
		if not Ent or not Ent:IsValid() or not ply or not ply:IsValid() or not DataToSend then
			return
		end
		-- Check duped
		...
		for k, v in ipairs(DataToSend) do
			...
			if v.remove == true then
				...
				if Ent.Scaling or Ent.TopLeft then
					...
					if original.VerticesUpdate then
						original.VerticesUpdate = false
					end
				end
	...
	function EGP:DoAction(Ent, E2, Action, ...)
		if Action == "SendObject" then
			local Data = { ... }
			local obj = Data[1]
			if not obj or obj._nodraw then
				return
			end
		...
		elseif Action == "RemoveObject" then
			...
			self:AddQueueObject(Ent, E2.player, SendObjects, {
				index = Data[1],
				remove = true,
			})
	...
	function EGP:Receive(netlen)
		...
		if not self:ValidEGP(Ent) or not Ent.RenderTable then
			return
		end
		...
		elseif Action == "ReceiveObjects" then
			...
			for i = 1, net.ReadUInt(16) do
				local index = net.ReadInt(16)
				if index == 0 then -- In case the umsg had to abort early
					break
				end
				...
				if ID == 0 then -- Remove object
					...
					if ChangeOrder_To ~= 0 then
						order_was_changed = true
						current_obj.ChangeOrder = {
							target = ChangeOrder_To,
							dir = ChangeOrder_Dir,
						}
					end
				end
	...
	net.Receive("EGP_Transmit_Data", function(netlen)
		EGP:Receive(netlen)
	end)
end
...
if SERVER then
	...
	concommand.Add("EGP_Request_Reload", function(ply, cmd, args)
		if not EGP.DataStream[ply] then
			EGP.DataStream[ply] = {}
		end
		local tbl = EGP.DataStream[ply]
		if not tbl.SingleTime then
			tbl.SingleTime = 0
		end
		if not tbl.AllTime then
			tbl.AllTime = 0
		end
		if args[1] then
	...
	function EGP:SendDataStream(ply, entid)
		if not ply or not ply:IsValid() then
			return false, "ERROR: Invalid ply."
		end
		local targets
		if entid then
		...
		if not targets then
			...
			if #targets == 0 then
				return false, "There are no EGP screens on the map."
			end
		end
