...
function EGP:NewObject(name, super)
	...
	if self.Objects[lower] then
		return self.Objects[lower]
	end
	-- Create table
	self.Objects[lower] = {}
	-- Set info
...
function EGP:HasObject(Ent, index)
	...
	if not Ent.RenderTable or #Ent.RenderTable == 0 then
		return false
	end
...
function EGP:SetOrder(ent, from, to, dir)
	...
	if ent.RenderTable[from] then
		...
		if SERVER then
			ent.RenderTable[from].ChangeOrder = {
				target = to,
				dir = dir,
			}
		end
