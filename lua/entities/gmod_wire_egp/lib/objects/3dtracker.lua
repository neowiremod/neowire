...
function Obj:Draw(egp)
	...
	if fraction < 0 then -- hide for fraction < 0
		self.x = math.huge
		self.y = math.huge
	elseif (fraction - 1) * self.directionality < 0 then
		-- hide for fraction > 1 if directionality < 0 and for fraction < 1 if directionality > 0
		self.x = math.huge
		self.y = math.huge
	else
...
function Obj:Receive()
	...
	if parententity and parententity:IsValid() then
		tbl.parententity = parententity
	end
