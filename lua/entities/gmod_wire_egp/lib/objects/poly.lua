...
function Obj:Contains(x, y)
	...
	if counterclockwise(vertices[1], vertices[2], vertices[3]) then
		...
		inside = function(a, b, c)
			return counterclockwise(b, a, c)
		end
	end

	for i = 1, #vertices - 1 do
		if not inside(vertices[i], vertices[i + 1], point) then
			return false
		end
	end
...
function Obj:EditObject(args)
	...
	if args.x or args.y or args.angle then
		...
		args.angle = nil
		if self._x then
			args._x, args._y, args._angle = nil, nil, nil
		end
	end
...
function Obj:SetPos(x, y, angle)
	...
	if sx == x and sy == y and sa == angle then
		return false
	end
