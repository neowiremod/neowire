	...
	if EGP:CacheNeedsUpdate(self, { "x", "y", "w", "h", "angle", "fidelity", "radius" }) then
		...
		for x = 1, 4 do
			for i = 0, (self.fidelity + 1) / 4 do
				...
				scx, scy = scx * cos(angle) - scy * sin(angle), scx * sin(angle) + scy * cos(angle)
				...
				local u, v = 0.5 + (dirUV.x * ru) + (srx / 2) * (1 - (ru * 2)), 0.5 + (dirUV.y * rv) + (sry / 2) * (1 - (rv * 2))
				polys[#polys + 1] = {
					x = xs + scx + (dir.x * r),
					y = ys + scy + (dir.y * r),
					u = u,
