...
function Obj:EditObject(args)
	...
	if args.x or args.y or args.angle or args.x2 or args.y2 then
		...
		args.angle = nil
		if args._x then
			args._x, args._x2, args._y, args._y2, args._angle = nil, nil, nil, nil, nil
		end
	end
...
function Obj:SetPos(x, y, angle, x2, y2)
	...
	if sx == x and sy == y and sa == angle and sx2 == x2 and sy2 == y2 then
		return false
	end
	local vec
	...
	if SERVER and self._x then
		self._x, self._y, self._angle, self._x2, self._y2 = x, y, angle, x2, y2
	end
	return true
end
return Obj
