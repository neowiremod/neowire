...
local MTLPARAM_TYPE = {
	...
	vector3 = {
		...
		"VECTOR",
		function(tbl)
			return Vector(tbl.x, tbl.y, tbl.z)
		end,
	},
	...
	matrix44 = {
		...
		"matrix4",
		function(tbl)
			return Matrix({ { tbl.a1, tbl.a2, tbl.a3, tbl.a4 }, { tbl.b1, tbl.b2, tbl.b3, tbl.b4 }, { tbl.c1, tbl.c2, tbl.c3, tbl.c4 }, { tbl.d1, tbl.d2, tbl.d3, tbl.d4 } })
		end,
	},
}
...
if SERVER then
	local InputsTable = { "Active", "Camera [ENTITY]", "Scroll X", "Scroll Y", "Scale X", "Scale Y" }
...
if CLIENT then
	...
	function ENT:DrawScreen()
		...
		v1 = v1 + scrV
		v2 = v2 + scrV
		cam.Start3D2D(self:LocalToWorld(monitor.offset), self:LocalToWorldAngles(monitor.rot), monitor.RS)
	...
	function ENT:Draw()
		...
		if self:GetActive() and self.ShouldRenderCamera and self.Material ~= nil and IsValid(self:GetCamera()) and self:GetCamera():GetActive() then
			self:DrawScreen()
