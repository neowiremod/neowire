...
function ENT:Setup(Force, Length, ShowBeam, Reaction)
	...
	if ShowBeam ~= nil then
		self:SetShowBeam(ShowBeam)
	end
	self:ShowOutput()
end
...
function ENT:Think()
	if self.Force == 0 and self.OffsetForce == 0 and self.Velocity == 0 then
		return
	end
	...
	if convar_value == 1 then
		if not IsValid(self:GetPlayer()) or gamemode.Call("GravGunPunt", self:GetPlayer(), ent) == false then
			return
		end
	elseif convar_value == 2 then
		if not IsValid(self:GetPlayer()) or gamemode.Call("GravGunPickupAllowed", self:GetPlayer(), ent) == false then
			return
		end
	end

	if hook.Run("Wire_ForcerCanUse", self:GetPlayer(), ent, self) == false then
		return
	end
	if ent:GetMoveType() == MOVETYPE_VPHYSICS then
		...
		if self.OffsetForce ~= 0 then
			phys:ApplyForceOffset(offsetForce, trace.HitPos)
		end
...
function ENT:ShowOutput()
	self:SetOverlayText(
		"Center Force = "
			.. math.Round(self.ForceMul * self.Force)
			.. "\nOffset Force = "
			.. math.Round(self.ForceMul * self.OffsetForce)
			.. "\nVelocity = "
			.. math.Round(self.Velocity)
			.. "\nLength = "
			.. math.Round(self:GetBeamLength())
	)
end
