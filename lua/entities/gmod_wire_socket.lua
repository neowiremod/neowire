...
function ENT:CanLink(Target)
	if Target.Socket and Target.Socket:IsValid() then
		return false
	end
	if self.SockData.plug ~= Target:GetModel() then
		return false
	end
	return true
end
...
function ENT:SetValue(name, value)
	if not (self.Plug and self.Plug:IsValid()) then
		return
	end
	if name == "In" then
