...
if CLIENT then
	...
	function ENT:GetWorldTipPositions(w, h, w_body, h_body, w_footer, h_footer)
		...
		if WireLib.WiringToolRenderAvoid then
			...
			if maxx - w < avoidMaxX and maxx > avoidMinX and maxy - h < avoidMaxY and maxy > avoidMinY then
	...
	hook.Add("HUDPaint", "wire_draw_world_tips", function()
		if not cl_drawworldtooltips:GetBool() then
			return
		end
		if SysTime() > WorldTip.dietime then
			return
		end
...
function ENT:SetOverlayText(txt)
	...
	if txt == self.OverlayData.txt then
		return
	end
