...
hook.Add("PlayerInitialSpawn", "WireMapInterface_PlayerInitialSpawn", function(ply)
	...
	for Ent, time in ipairs(Ents) do
		...
		timer.Simple(time + 0.1, function()
			...
			if not Ent.GiveWireInterfeceClient then
				return
			end
...
function ENT:IsWireableEntity(Entity)
	if not IsValid(Entity) then -- No interface for invalid entities!
		return false
	end

	if IsValid(Entity._WireMapInterfaceEnt) and (Entity._WireMapInterfaceEnt ~= self) then -- Only one interface per entity!
		return false
	end

	if not IsValid(Entity._WireMapInterfaceEnt) and (WireLib.HasPorts(Entity) or Entity.IsWire or Entity.Inputs or Entity.Outputs) then -- Don't destroy wiremod entites!
		return false
	end

	if Entity:IsWorld() then -- No interface for the worldspawn!
		return false
	end

	if Entity:IsVehicle() then -- No interface for vehicles!
		return false
	end

	if Entity:IsNPC() then -- No interface for NPCs!
		return false
	end

	if Entity:IsPlayer() then -- No interface for players!
		return false
	end

	if Entity:IsWeapon() then -- No interface for weapons!
		return false
	end

	if string.match(Entity:GetClass(), "^(item_[%w_]+)") then -- No interface for items!
		return false
	end

	if Entity:IsConstraint() then -- No interface for constraints!
		return false
	end

	if Entity:GetPhysicsObjectCount() > 1 then -- No interface for ragdolls!
		return false
	end

	if IsValid(Entity:GetPhysicsObject()) then -- Everything with a single physics object can get an interface!
		return true
	end
	return true
end
...
local function RunLua(I, name, value, wired, self, Ent)
	...
	if (lua == "") or not self.RunLuaCode then
		return
	end
...
function ENT:TriggerWireInput(name, value, wired, Ent)
	...
	if ((CurTime() - (self.Timer.In[name] or 0)) < (self.min_trigger_time or MIN_TRIGGER_TIME)) and not WireRemoved then
		return
	end
	self.Timer.In[name] = CurTime()
	self.Data = self.Data or {}
	self.Data.In = self.Data.In or {}
	if (self.Data.In[name] == value) and not WireRemoved then
		return
	end
...
function ENT:AcceptInput(name, activator, caller, data)
	...
	if self.Active and ALLOW_INTERFACE:GetBool() then
		...
		if (I > 0) and (I <= MAX_PORTS) and self.OutsExist[I] then
			...
			if (CurTime() - (self.Timer.Out[name] or 0)) < (self.min_trigger_time or MIN_TRIGGER_TIME) then
				return false
			end
	...
	if not self.WirePortsChanged then
		return false
	end
	if name == "addentity" then
		local Ent, Func = self:AddSingleEntity(caller)
		if not IsValid(Ent) or not Func then
			return false
		end
