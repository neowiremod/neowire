...
function ENT:GiveWireInterfece(EntsToAdd)
	if not EntsToAdd then
		return
	end
	if table.IsEmpty(EntsToAdd) or not self.WirePortsChanged then
		return
	end
	local AddedEnts = {}
	self:UpdateData()
	self:Timedpairs("WireMapInterface_Adding", EntsToAdd, 1, function(obj1, obj2, self)
		if not IsValid(self) then -- Stop loop when the entity gets removed.
			return false
		end
		...
		if Ent == "limid_exceeded" then -- Stop loop when maximum got exceeded
			return false
		end
...
function ENT:GiveWireInterfeceClient(ply, EntsToAdd)
	...
	self:Timedpairs((IsValid(ply) and (ply:EntIndex() .. "") or "") .. "WireMapInterface_Adding_CL", EntsToAdd or self.WireEnts, 1, function(Entity, Func, self)
		...
		if not self:IsWireableEntity(Entity) then
			return
		end
...
local function AddSingleEntityCL(self, Entity, ply, SendToAll)
	...
	if not self.WireEnts[Entity] then
		return
	end
	if not SendToAll and not IsValid(ply) then
		return
	end
	if SendToAll then
		umsg.Start("WireMapInterfaceEnt")
...
function ENT:AddSingleEntity(Entity, callOnEnd, AddedEnts)
	...
	if not self:CheckEntLimid(callOnEnd, AddedEnts) then
		return "limid_exceeded"
	end
...
function ENT:RemoveEntitiesByTable(Table, callback)
	...
	if table.IsEmpty(Table) or not self.WirePortsChanged then
		return
	end
	local Removed = nil
	self:Timedpairs("WireMapInterface_Removing", Table, 1, function(obj1, obj2, self)
		...
		if not IsValid(Entity._WireMapInterfaceEnt) then
			return
		end
		if Entity._WireMapInterfaceEnt ~= self then
			return
		end
