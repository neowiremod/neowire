...
function ENT:DoJob()
	...
	if self.Command[8] ~= 0 then
		...
		if self.Command[10] ~= 0 then
			if self.Command[11] ~= self.Command[7] then
				dojob = false
			end
		end
		...
		if dojob then
			...
			if self.Command[0] ~= 0 then --write ray
				...
				self.WriteBuffer = table.Copy(disk.DiskMemory[sector_addr]) or {
					[0] = 0,
				}
			end
		end
...
function ENT:Think()
	...
	if trace.Entity and trace.Entity:IsValid() and (trace.Entity:GetClass() == "gmod_wire_cd_disk") then
		...
		if (track >= disk.FirstTrack) and (stack >= 0) and (sector >= 0) and (stack < disk.DiskStacks) then --(track < disk.DiskTracks) and
			...
			if not disk.TrackSectors[track] then
				disk.TrackSectors[track] = 0
			end
