...
function ENT:Setup(xyz_mode, outdist, outbrng, gpscord, direction_vector, direction_normalized, target_velocity, velocity_normalized)
	if not xyz_mode and not outdist and not outbrng and not gpscord and not direction_vector and not target_velocity then
		outdist = true
	end
...
function ENT:Think()
	...
	if not IsValid(self.ToSense) or not self.ToSense.GetBeaconPos then
		return
	end
	if self.Active then
		local dist = 0
		...
		if self.direction_vector then
			...
			if self.direction_normalized then
				dirvec:Normalize()
			end
		end
		...
		if self.target_velocity then
			...
			if self.velocity_normalized then
				velo:Normalize()
			end
		end
