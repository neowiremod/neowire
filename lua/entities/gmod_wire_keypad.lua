...
if CLIENT then
	...
	function ENT:Draw()
		...
		if (Ply:GetShootPos() - self:GetPos()):Length() > 750 then
			return
		end
...
net.Receive("wire_keypad", function(netlen, ply)
	...
	if not IsValid(ent) or not ent.Password then
		return
	end
	if ent.CurrentNum == -1 then -- Display still shows ACCESS from a past success
		return
	end

	if (ply:GetShootPos() - ent:GetPos()):Length() > 50 then
		return
	end
