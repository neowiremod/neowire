...
function ENT:WriteCell(Address, value)
	...
	-- 2040 - Hardware Clear Column (Writing clears column)
	-- 2041 - Hardware Clear Screen
	if (Address == 2025) or (Address == 2026) or (Address == 2027) or (Address == 2028) or (Address == 2036) then
		self.NeedRefresh = true
	end
...
function ENT:DrawSpecialCharacter(c, x, y, w, h, r, g, b)
	...
	if vertices then
		local tf = Matrix()
		tf:SetScale(Vector(w, h, 1))
		tf:SetTranslation(Vector(x, y, 0))
				...
				for tx = 0, 29 do
					...
					if (c1 ~= 0) and (cfrnt ~= 0) then
						...
						if specialCharacters[c1] then
							self:DrawSpecialCharacter(c1, (tx + 0.5) * szx, (ty + 0.5) * szy, szx, szy, fr, fg, fb)
						else
							draw.DrawText(utf8, "WireGPU_ConsoleFont", (tx + 0.625) * szx, (ty + 0.75) * szy, Color(fr, fg, fb, 255), 0)
						end
					end
				end
			end

			if self.Memory1[2045] > 1080 then
				self.Memory1[2045] = 1080
			end
			if self.Memory1[2045] < 0 then
				self.Memory1[2045] = 0
			end
			if self.Memory1[2044] > 1 then
				self.Memory1[2044] = 1
			end
			if self.Memory1[2044] < 0 then
				self.Memory1[2044] = 0
			end
			if self.Memory1[2046] >= 1 then
				if self.Flash == true then
					...
					surface.DrawRect(tx * szx + szx / 2, ty * szy + szy / 2 + szy * 1.2 * (1 - self.Memory1[2044]), szx * 1.2, szy * 1.2 * self.Memory1[2044])
				end
			end
		end)
	end
