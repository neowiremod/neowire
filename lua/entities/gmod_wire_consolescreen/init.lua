...
function ENT:SendPixel()
	if (self.Memory[2047] ~= 0) and (self.CharX >= 0) and (self.CharX < 30) and (self.CharY >= 0) and (self.CharY < 18) then
...
function ENT:ReadCell(Address)
	...
	if Address == 2022 then
		return WireGPU_Monitors[self:GetModel()].RatioX
	end
...
function ENT:WriteCell(Address, value)
	...
	if Address < 2000 then -- text/attribute data
		if self.Memory[Address] == value then
			return true
		end
	else
...
function ENT:ClientWriteCell(Address, value)
	...
	elseif Address == 2037 then
		-- Shift cells (number of cells, >0 right, <0 left)
	...
	elseif Address == 2038 then
		-- Shift rows (number of rows, >0 shift down, <0 shift up)
		local delta = math.floor(math.Clamp(math.abs(value), -30, 30))
		local low = math.floor(math.Clamp(self.Memory[2033], 0, 17))
		local high = math.floor(math.Clamp(self.Memory[2034], 0, 17))
		local lowc = math.floor(math.Clamp(self.Memory[2031], 0, 29))
	...
	elseif Address == 2039 then
		-- Hardware Clear Row (Writing clears row)
		for i = 0, 59 do
			self.Memory[value * 60 + i] = 0
		end
	elseif Address == 2040 then
		-- Hardware Clear Column (Writing clears column)
		for i = 0, 17 do
			self.Memory[i * 60 + value] = 0
		end
	elseif Address == 2041 then
