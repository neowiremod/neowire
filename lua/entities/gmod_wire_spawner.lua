...
hook.Add("EntityRemoved", "wire_spawner_EntityRemoved", function(ent)
	if not GlobalUndoList[ent] then
		return
	end
...
function ENT:EndTouch(ent)
	if ent.PropSpawner == self then
		self.DisabledByTouch = false
	end
end

function ENT:DoSpawn(pl, down)
	if self.DisabledByTimeUntil > CurTime() then
		return
	end
	if self.DisabledByTouch then --If we'll teleport spawned prop fast prop spawner will remain disabled forever (Fix by Supertos)
		...
		if IsValid(self.TouchDisabledProp) then --If there's no prop why should we even care?
			...
			if self.TouchDisabledProp:GetPos():Distance(self:GetPos()) < test_radius * 2 then --This prop is still inside prop spawner (Multiple by two because spawned prop has some volume too )
				return
			end
		end
	end
...
function ENT:DoUndo(pl)
	if not next(self.UndoList) then
		return
	end
...
function ENT:DoUndoEnt(pl, ent)
	...
	if GlobalUndoList[ent] ~= self then
		return
	end
	ent:Remove()
	WireLib.AddNotify(pl, "Undone Prop", NOTIFY_UNDO, 2)
...
function ENT:TriggerInput(iname, value)
	...
	if iname == "Spawn" then
		...
		if SpawnThisValue == self.SpawnLastValue then
			return
		end
	...
	elseif iname == "Undo" then
		...
		if UndoThisValue == self.UndoLastValue then
			return
		end
		self.UndoLastValue = UndoThisValue
		if UndoThisValue then
			self:DoUndo(pl)
		end
