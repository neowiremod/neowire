...
function ENT:StartTouch(ent)
	...
	if ent == owner then -- this never happens but just in case...
		return
	end

	if owner:GetFilter() == 1 and not ent:IsPlayer() or owner:GetFilter() == 2 and ent:IsPlayer() then
		return
	end
	local ply = ent:IsPlayer() and ent
	if owner:GetOwnerOnly() and (WireLib.GetOwner(ent) or ply) ~= WireLib.GetOwner(owner) then
		return
	end
