...
if CLIENT then
	...
	function ENT:Think()
		...
		for k = #self.Points, 1, -1 do
			...
			if k == #self.Points and keeplatest:GetBool() then -- Check keep latest convar
				continue
			end
			...
			if v.DieTime then
				...
				if v.DieTime < CurTime() then -- If the point's time has passed, remove it
					...
					if self.Points[k - 1] then -- Don't draw a line to this point anymore
						self.Points[k - 1].LineBeam = false
					end
				end
			end
	...
	function ENT:Draw()
		...
		local n = #self.Points
		if n == 0 or self:GetNWBool("Active", true) == false then
			return
		end
		for k = 1, n do
			local v = self.Points[k]
			...
			if v.GroundBeam then
				...
				render.DrawBeam(selfpos, Pos, v.Size, 0, 1, v.Color)
			end
			...
			if v.LineBeam and k < n then
				...
				render.DrawBeam(NextPos, Pos, v.Size * 2, 0, 1, v.Color)
			end

			render.SetMaterial(matpoint)
			render.DrawSprite(Pos, v.Size, v.Size, v.Color)
		end
	end
...
function ENT:Initialize()
	...
	self.Outputs = WireLib.CreateOutputs(self, {
		"Memory (Allows zGPU/zCPU to communicate with this device. Serves no other purpose.)", -- Compatibility for older hispeed devices (such as gpu/cpu)
	})
