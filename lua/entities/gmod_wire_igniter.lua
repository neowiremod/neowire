...
function ENT:DoTrace()
	...
	if not gamemode.Call("CanProperty", self:GetPlayer(), "ignite", trace.Entity) then
		return false
	end
	if trace.Entity:IsPlayer() and (not self.TargetPlayers or not cvarTargetPlayers) then
		return false
	end
