...
function ENT:WriteCell(Address, value)
	...
	if Address == 1009 then -- Screen width
		...
		if value * mem[1010] > 1003 or value * 18 > 1024 then
			return false
		end
	elseif Address == 1010 then
		-- Screen height
		value = math.floor(value)
		if value * mem[1009] > 1003 or value * 24 > 1024 then
			return false
		end
	elseif Address == 1011 then
	...
	elseif Address == 1017 then
		value = math.floor(value)
		if value < 0 or value >= mem[1010] then
			return false
		end
				...
				for tx = 0, mem[1009] - 1 do
					...
					if c1 ~= 0 then
						...
						if specialCharacters[c1] then
							self:DrawSpecialCharacter(c1, tx * szx + 1, ty * szy + 1, szx - 1, szy - 1, fr, fg, fb)
						else
							draw.DrawText(utf8, "LCDFontBlur", tx * szx + 2, ty * szy, Color(fr, fg, fb, 255), 0)
							draw.DrawText(utf8, "LCDFont", tx * szx + 1, ty * szy - 1, Color(fr, fg, fb, 255), 0)
						end
					end
				end
			end
		...
		if self.IntTimer >= mem[1019] then
			if self.IntTimer >= mem[1019] * 2 then
				...
				self.GPU:RenderToGPU(function()
					...
					local fg = (1 - fc) * mem[1004] + fc * mem[1007]
					local fb = (1 - fc) * mem[1005] + fc * mem[1008]
					surface.SetDrawColor(fr, fg, fb, 255)
					surface.DrawRect(tx * szx + 1, ty * szy + szy * (1 - mem[1020]) - 1, szx - 2, szy * mem[1020])
				end, nil, true)
			end
		end
	end
