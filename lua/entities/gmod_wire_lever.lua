...
if CLIENT then
	...
	function ENT:Think()
		...
		if isClicking and not self.wasClicking and IsValid(self.csmodel) then
			...
			if aimPos:DistToSqr(self:GetPos()) < 100 ^ 2 then
				local rayPos =
					util.IntersectRayWithOBB(aimPos, LocalPlayer():GetAimVector() * 100, self.csmodel:GetPos(), self.csmodel:GetAngles(), self.csmodel:OBBMins() - Vector(2, 2, 2), self.csmodel:OBBMaxs() + Vector(2, 2, 2))
				if rayPos then
	...
	function ENT:Use(ply)
		if not IsValid(ply) or not ply:IsPlayer() or IsValid(self.User) then
			return
		end
		self.User = ply
	...
	net.Receive("wire_lever_activate", function(netlen, ply)
		...
		if not IsValid(ply) or not IsValid(ent) or not ent.Use or ent:GetClass() ~= "gmod_wire_lever" then
			return
		end
		if IsValid(ent.User) then
			return
		end
	...
	function ENT:ConvertFromOldLever(base)
		...
		timer.Simple(0, function()
			-- give the setpos time to be applied
	...
	local fix_after_dupe = setmetatable({}, {
		__mode = "kv",
	})

	hook.Add("AdvDupe_FinishPasting", "LeverFixOldDupe", function(data)
		if next(fix_after_dupe) == nil then
			return
		end
		local levers = {}
