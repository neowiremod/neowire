	...
	if AutoMove then
		...
		if ply.LocalEyeAngles then
			...
			if SERVER and IsValid(veh) then
				curang = veh:WorldToLocalAngles(curang)
			end
		end
...
if CLIENT then
	...
	hook.Add("InputMouseApply", "wire_camera_controller_input_unlock", function(cmd, x, y, ang)
		...
		if unroll then
			...
			if hasParent then
				...
				if math.abs(diffAngles.y) > 90 then
					diffAngles.r = -diffAngles.r
				end
	...
	net.Receive("wire_camera_controller_toggle", function(len)
		...
		if cam ~= self and enabled then -- another camera controller is already enabled
			return
		end
	...
	net.Receive("wire_camera_controller_sync", function(len)
		if not enabled or not IsValid(self) then
			return
		end
		local cam = net.ReadEntity()
		if cam ~= self then -- another cam controller is trying to hijack us
			return
		end
...
function ENT:UpdateOverlay()
	...
	if self.AutoUnclip_IgnoreWater then
		unclip = unclip .. " (Ignores water)"
	end
	self:SetOverlayText(
		string.format(
...
function ENT:GetContraption()
	if CurTime() < self.NextGetContraption then
		return
	end
...
function ENT:UpdateOutputs()
	if CurTime() < self.NextUpdateOutputs then
		return
	end
	...
	if self.Active and IsValid(ply) then
		...
		local trace = util.TraceLine({
			start = curpos,
			endpos = curpos + curang:Forward() * 999999999,
			filter = self.Entities,
		})
...
hook.Add("SetupPlayerVisibility", "gmod_wire_cameracontroller", function(player)
	...
	if IsValid(cam) then
		...
		if IsValid(cam.Parent) then
			pos = cam.Parent:LocalToWorld(pos)
		end
		AddOriginToPVS(pos)
	end
end)
...
function ENT:EnableCam(ply)
	...
	if self.DuplicationInProgress then
		return
	end
	if #self.Vehicles == 0 and not ply then -- if the cam controller isn't linked, it controls the owner's view
		ply = self:GetPlayer()
	...
	if IsValid(ply) then
		for i = 1, #self.Players do
			if self.Players[i] == ply then -- check if this player is already active
				return
			end
		end
...
function ENT:SetFOV(ply, b)
	if b == nil and self.FOV ~= nil then
		b = true
	end
	if self.FOV == 0 then
		b = false
...
function ENT:WriteCell(address, value)
	if not hispeed_ports[address] then
		return false
	end
	local key = hispeed_ports[address]
	if address < 14 then
		if address == 2 then -- special case: parent entity by entid
			value = Entity(value)
		end
...
function ENT:ApplyDupeInfo(ply, ent, info, GetEntByID)
	...
	if info.cam or info.pod or info.OldDupe then -- OLD DUPE DETECTED
		...
		WireLib.AdjustSpecialInputs(self, { "Activated", "X", "Y", "Z", "Pitch", "Yaw", "Roll", "Angle [ANGLE]", "Position [VECTOR]", "Distance", "UnRoll", "Direction [VECTOR]", "Parent [ENTITY]", "FLIR", "FOV" })
		WireLib.AdjustSpecialOutputs(self, { "On", "X", "Y", "Z", "HitPos [VECTOR]", "CamPos [VECTOR]", "CamDir [VECTOR]", "CamAng [ANGLE]", "Trace [RANGER]" })
		self.OldDupe = true
	else
	...
	timer.Simple(0.1, function()
		if IsValid(self) then -- timers solve everything (the entity isn't valid on the client at first, so we wait a bit)
			self:UpdateMarks()
		end
	end)
end
