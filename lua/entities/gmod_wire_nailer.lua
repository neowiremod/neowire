...
function ENT:CanNail(trace)
	...
	if not WireLib.CanTool(self:GetPlayer(), trace.Entity, "nailer") then
		return false
	end
	return true
end
...
function ENT:TriggerInput(name, value)
	...
	if not self:CanNail(trace1) then
		return
	end
	...
	if not self:CanNail(trace2) then
		return
	end
	if name == "Weld" then
		constraint.Weld(trace1.Entity, trace2.Entity, trace1.PhysicsBone, trace2.PhysicsBone, self.Flim)
	elseif name == "Axis" then
		...
		constraint.Axis(trace1.Entity, trace2.Entity, trace1.PhysicsBone, trace2.PhysicsBone, LPos1, LPos2, self.Flim)
	elseif name == "Ballsocket" then
		constraint.Ballsocket(trace1.Entity, trace2.Entity, trace1.PhysicsBone, trace2.PhysicsBone, trace2.Entity:WorldToLocal(trace1.HitPos), self.Flim)
	end
