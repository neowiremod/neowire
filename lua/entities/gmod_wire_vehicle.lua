...
function ENT:LinkEnt(pod)
	...
	if not WireLib.CanTool(self:GetPlayer(), pod, "wire_vehicle") then
...
function ENT:Think()
	if IsValid(self.Vehicle) then
		...
		if self.Steering then
			self.Vehicle:Fire("steer", self.Steering + delta, 0)
		end
		if self.Throttle then
			self.Vehicle:Fire("throttle", self.Throttle + delta, 0)
		end
	end
