...
local BalloonTypes = { Model("models/MaxOfS2D/balloon_classic.mdl"), Model("models/balloons/balloon_classicheart.mdl"), Model("models/balloons/balloon_dog.mdl"), Model("models/balloons/balloon_star.mdl") }
CreateConVar("sbox_maxwire_deployers", 2)
local DmgFilter
local function CreateDamageFilter()
	if IsValid(DmgFilter) then
		return
	end
...
local function MakeBalloonSpawner(pl, Data)
	if IsValid(pl) and not pl:CheckLimit("wire_deployers") then
		return nil
	end
...
function ENT:SpawnFunction(ply, tr)
	...
	local ent = MakeBalloonSpawner(ply, {
		Pos = SpawnPos,
	})
	return ent
end
...
function ENT:DeployBalloons()
	...
	duplicator.DoGeneric(balloon, {
		Pos = self:GetPos() + (self:GetUp() * 25),
	})

	duplicator.DoGenericPhysics(balloon, pl, {
		Pos = Pos,
	})
	...
	if self.weld then
		...
		local constraint, rope = constraint.Rope(balloon, hitEntity, 0, trace.PhysicsBone, balloonPos, hitPos, 0, self.rl, 0, 1.5, material, false)
		if constraint then
			balloon:DeleteOnRemove(constraint)
