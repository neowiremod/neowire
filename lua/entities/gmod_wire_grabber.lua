...
function ENT:LinkEnt(prop)
	if not IsValid(prop) then
		return false, "Not a valid entity!"
	end
	self.ExtraProp = prop
...
function ENT:CanGrab(trace)
	if not trace.Entity or not isentity(trace.Entity) then
		return false
	end
	if (not trace.Entity:IsValid() and not trace.Entity:IsWorld()) or trace.Entity:IsPlayer() then
		return false
	end
	...
	if not WireLib.CanTool(self:GetPlayer(), trace.Entity, "weld") then
		return false
	end
	return true
end
...
function ENT:TriggerInput(iname, value)
	if iname == "Grab" then
		if value ~= 0 and self.Weld == nil then
			...
			if not self:CanGrab(trace) then
				return
			end
			-- Weld them!
