...
for i = 17, 20 do -- arrow keys
	unprintable_chars[i] = true
end

for i = 127, 177 do -- backspace, numpad, ctrl, alt, shift, break, F1-F12, scroll/num/caps lock, and more
	unprintable_chars[i] = true
end
...
local function getPrintableChar(key)
	if convertable_chars[key] then
		return convertable_chars[key]
	end
	if unprintable_chars[key] then
		return ""
	end
	if key == 13 then -- convert newline '13' into newlne '10' to make it work properly
		key = 10
	end
...
function ENT:Initialize()
	...
	self.Outputs = WireLib.CreateOutputs(
		self,
		{
...
function ENT:TriggerOutputs(key)
	...
	str = str
		...
		.. (self.Synchronous and "Yes" or "No")
		.. "\n"
		.. "Automatic buffer clear: "
		.. (self.AutoBuffer and "Yes" or "No")
		.. "\n"
		.. "Enter key ASCII output: "
		.. (self.EnterKeyAscii and "10 ('\\n')" or "13 ('\\r')")
...
function ENT:PlayerAttach(ply)
	if not IsValid(ply) or IsValid(self.ply) then -- If the keyboard is already in use, don't attach the player
		return
	end

	if IsValid(ply.WireKeyboard) then -- If the player is already using a different keyboard
		if ply.WireKeyboard == self then -- If the keyboard is this keyboard, don't re-attach the player
			return
		end
...
function ENT:GetRemappedKey(key_enum)
	if not key_enum or key_enum == 0 or key_enum > KEY_LAST then -- Above KEY_LAST are joystick and mouse enums
		return 0
	end
...
function ENT:RemoveFromBufferByKey(key)
	local key_enum = All_Enums[key]
	if not key_enum then -- key is invalid
		return false
	end

	local positions = self.BufferLookup[key_enum]
	if not positions then -- error, shouldn't happen
		return false
	end

	local bufferpos = table.remove(positions, 1)
	if not bufferpos then -- error, shouldn't happen
		return false
	end
