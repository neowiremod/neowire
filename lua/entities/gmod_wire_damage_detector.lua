...
hook.Add("EntityTakeDamage", "CheckWireDamageDetectors", function(ent, dmginfo)
	if not next(Wire_Damage_Detectors) then
		return
	end
...
function ENT:ShowOutput()
	...
	self:SetOverlayText(
		string.format("(%s)\nLinked to %s entities%s", (self.includeconstrained == 0) and "Individual Props" or "Constrained Props", num, (num == 1 and self.linked_entities[1] == self) and "\nLinked only to self" or "")
	)
end
...
function ENT:LinkEnt(ent, dontupdateoutput)
	...
	if self.linked_entities_lookup[ent] then
		return false
	end
...
function ENT:UnlinkEnt(ent)
	if not self.linked_entities_lookup[ent] then
		return false
	end
...
function ENT:TriggerInput(iname, value)
	...
	elseif iname == "Entities" then
		...
		if value then
			if self.arrayInputNextChange and self.arrayInputNextChange > CurTime() then
				return
			end
			self:ClearEntities()
...
function ENT:UpdateDamage(dmginfo, ent) -- Update damage table
	...
	if self.count == math.huge then -- This shouldn't ever happen... unless you're really REALLY bored
		self.count = 0
	end
end
