...
local WidgetBuilders = {
	...
	DNumberScratch = function(self, data, body, index)
		...
		numberscratch.Paint = function(self, w, h)
			...
			surface.DrawLine(w / 2, h / 2, math.sin(value * math.pi * 0.75) * w / 2 + w / 2, -math.cos(value * math.pi * 0.75) * h / 2 + h / 2)
		end
	end,
...
net.Receive("wire_interactiveprop_action", function(len, ply)
	...
	if not ent:IsValid() or ent:GetClass() ~= "gmod_wire_interactiveprop" or ply ~= ent.User then
		return
	end
	ent:ReceiveData()
	ent:UpdateOverlay()
