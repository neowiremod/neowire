...
function ENT:Setup(key, damage, delaytime, removeafter, radius, affectother, notaffected, delayreloadtime, maxhealth, bulletproof, explosionproof, fallproof, explodeatzero, resetatexplode, fireeffect, coloreffect, invisibleatzero)
	...
	if self.Damage > 0 then
		self.NormInfo = self.NormInfo .. "Damage: " .. self.Damage
	end
	if self.Radius > 0 or self.Delaytime > 0 then
		self.NormInfo = self.NormInfo .. "\n"
	end
	if self.Radius > 0 then
		self.NormInfo = self.NormInfo .. " Rad: " .. self.Radius
	end
	if self.Delaytime > 0 then
		self.NormInfo = self.NormInfo .. " Delay: " .. self.Delaytime
	end
	self:ShowOutput()
	local ttable = {
		key = key,
...
function ENT:OnTakeDamage(dmginfo)
	if dmginfo:GetInflictor():GetClass() == "gmod_wire_explosive" and not self.Affectother then
		return
	end
	if not self.Notaffected then
		self:TakePhysicsDamage(dmginfo)
	end
	if (dmginfo:IsBulletDamage() and self.BulletProof) or (dmginfo:IsExplosionDamage() and self.ExplosionProof) or (dmginfo:IsFallDamage() and self.FallProof) then --fix fall damage, it doesn't happen
		return
	end
...
function ENT:Trigger()
	if self.Delaytime > 0 then
		...
		if self.FireEffect then
			self:Ignite(self.Delaytime + 3, 0)
		end
	end
...
function ENT:Explode()
	...
	self:Extinguish()
	if not self.exploding then --why are we exploding if we shouldn't be
		return
	end
