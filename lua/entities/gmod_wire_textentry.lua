...
if CLIENT then
	...
	net.Receive("wire_textentry_show", function()
		...
		panel = Derma_StringRequestNoBlur("Wire Text Entry", "Enter text below", "", function(text)
...
function ENT:Initialize()
	...
	self.Outputs = WireLib.CreateOutputs(self, { "In Use", "Text [STRING]", "User [ENTITY]", "Entered (Set to 1 for a bit when text is successfully entered)" })
...
function ENT:UnlinkEnt(ent)
	if not IsValid(ent) then
		return false, "Invalid entity specified"
	end
...
function ENT:LinkEnt(ent)
	...
	if not ent:IsVehicle() then
		return false, "Entity must be a vehicle"
	end
...
net.Receive("wire_textentry_action", function(len, ply)
	local self = net.ReadEntity()
	if not IsValid(self) or not IsValid(ply) or ply ~= self.User then
		return
	end
...
function ENT:Prompt(ply)
	if ply then
		if CurTime() < self.NextPrompt then -- anti spam
			return
		end
...
function ENT:Use(ply)
	if self:GetDisableUse() or not IsValid(ply) then
		return
	end
	self:Prompt(ply)
end
