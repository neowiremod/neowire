			...
			hook.Add("HUDShouldDraw", "Wire pod HUDShouldDraw", function(name)
				if hideHUD > 0 then
					if LocalPlayer():InVehicle() then
						...
						if name ~= "CHudCrosshair" and name ~= "CHudGMod" and (hideHUD > 1 and name == "CHudChat" or name ~= "CHudChat") then
							return false
						end
					else
						hideHUD = 0
...
function ENT:Initialize()
	...
	local outputs = {
		...
		"AimPos [VECTOR]",
		"Distance",
		"Bearing (If the 'Relative' input is non-zero, this will be relative to the vehicle.)",
		"Elevation (If the 'Relative' input is non-zero, this will be relative to the vehicle.)",
		-- Other info
		"ThirdPerson",
		"Team",
		"Health",
	...
	local inputs = {
		...
		"Damage Health (Damages the driver's health.)",
		"Damage Armor (Damages the driver's armor.)",
		"Hide Player",
		"Hide HUD",
		"Show Cursor",
		"Vehicle [ENTITY]",
...
function ENT:SetLocked(b)
	if not self:HasPod() or self.Locked == b then
		return
	end
	self.Locked = b
...
function ENT:LinkEnt(pod)
	...
	if not WireLib.CanTool(self:GetPlayer(), pod, "wire_pod") then
...
function ENT:HasPod()
	return self.Pod and self.Pod:IsValid()
end
...
function ENT:SetPod(pod)
	if pod and pod:IsValid() and not pod:IsVehicle() then
		return false
	end
	if self:HasPly() then
		self:PlayerExited()
...
function ENT:TriggerInput(name, value)
	if name == "Lock" then
		...
		if not self:HasPod() then
			return
		end
		self:SetLocked(value ~= 0)
	elseif name == "Terminate" then
		if value == 0 or not self:HasPly() then
			return
		end
	...
	elseif name == "Damage Health" then
		if not self:HasPly() or value <= 0 then
			return
		end
	...
	elseif name == "Vehicle" then
		if not IsValid(value) then -- only link if the input is valid. that way, it won't be unlinked if the wire is disconnected
			return
		end
...
function ENT:Think()
	if self:HasPly() and self.Activated then
		...
		local trace = util.TraceLine({
			...
			endpos = ply:GetShootPos() + ply:GetAimVector() * 9999999999,
			filter = { ply, pod },
		})

		local distance
		if self:HasPod() then
			distance = trace.HitPos:Distance(pod:GetPos())
		else
			distance = trace.HitPos:Distance(ply:GetShootPos())
		end
...
function ENT:Use(User, caller)
	...
	if not hook.Run("PlayerGiveSWEP", User, "remotecontroller", weapons.Get("remotecontroller")) then
		return
	end
	...
	timer.Create("pod_use_" .. self:EntIndex(), 2, 1, function()
		...
		if not User:KeyDown(IN_USE) then
			return
		end
		if not User:GetEyeTrace().Entity or User:GetEyeTrace().Entity ~= self then
			return
		end
