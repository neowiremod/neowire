...
function ENT:Initialize()
	...
	self.nocollide_masks = {
		...
		{
			false, -- 1 nocollide between the two
			false,
			true,
		},
		{
			true, -- 2 nocollide Ent1 with all
			false,
			false,
		},
		{
			false, -- 3 nocollide Ent2 with all
			true,
			false,
		},
		{
			true, -- 4 nocollide both with all
			true,
			false,
		},
	}
	...
	self.nocollide_description = { "NoCollided", "Ent1 has collisions disabled", "Ent2 has collisions disabled", "All collisions disabled" }
	self.Nocollide = nil
	...
	elseif iname == "NoCollide" then
		...
		if IsValid(self.Ent1) then
			...
			if phys:IsValid() then
				phys:EnableCollisions(not mask[1])
			end
		end
		...
		if IsValid(self.Ent2) then
			...
			if phys:IsValid() then
				phys:EnableCollisions(not mask[2])
			end
		end
...
function MakeWireLatch(Ent1, Ent2, Bone1, Bone2, forcelimit)
	if not constraint.CanConstrain(Ent1, Bone1) then
		return false
	end
	if not constraint.CanConstrain(Ent2, Bone2) then
		return false
	end
