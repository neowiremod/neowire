...
if CLIENT then
	...
	function ENT:Think()
		...
		if ang.p < 0 and not self:GetOut180() then
			ang.p = ang.p + 360
		end
		if ang.y < 0 and not self:GetOut180() then
			ang.y = ang.y + 360
		end
		if ang.r < 0 and not self:GetOut180() then
			ang.r = ang.r + 360
		elseif ang.r > 180 and self:GetOut180() then
			ang.r = ang.r - 360
		end
