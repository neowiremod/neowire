...
function ENT:Initialize()
	...
	self.Inputs = WireLib.CreateInputs(self, { "Memory1", "Memory2", "Memory3", "Memory4", "Memory5", "Memory6", "Memory7", "Memory8" })
	self.DataRate = 0
	self.DataBytes = 0
...
function ENT:ReadCell(Address)
	...
	if (Address >= 0) and (Address < self.ControlDataSize) then
		...
		for i = 1, 8 do
			if (Address - self.ControlDataSize >= self.MemStart[i]) and (Address - self.ControlDataSize <= self.MemEnd[i]) then
...
function ENT:GetDeviceInfo(deviceEnt)
	...
	if self.PerformRecursiveScan >= 1 then
		...
		if recursiveCounter < 256 then
			if (deviceEnt:GetClass() == "gmod_wire_addressbus") or (deviceEnt:GetClass() == "gmod_wire_extbus") then
				for i = 1, 8 do
