...
function ENT:ClutchExists(Ent1, Ent2)
	for k, _ in pairs(self.clutch_ballsockets) do
		if (Ent1 == k.Ent1 and Ent2 == k.Ent2) or (Ent1 == k.Ent2 and Ent2 == k.Ent1) then
			return true
		end
	end
	return false
...
local function NewBallSocket(Ent1, Ent2, friction)
	...
	local ballsocket = constraint.AdvBallsocket(Ent1, Ent2, 0, 0, Vector(0, 0, 0), Vector(0, 0, 0), 0, 0, -180, -180, -180, 180, 180, 180, friction, friction, friction, 1, 0)
	if ballsocket then
...
function ENT:TriggerInput(iname, value)
	if iname == "Friction" then
		if not self.ClutchDelay then
			...
			timer.Create("wire_clutch_delay_" .. tostring(self:EntIndex()), Delay, 0, function()
				ClutchDelayEnd(self)
			end)
		else
...
function ENT:ApplyDupeInfo(ply, ent, info, GetEntByID)
	...
	for _, v in pairs(info.constrained_pairs) do
		...
		if IsValid(Ent1) and Ent1 ~= Ent2 and WireLib.CanTool(ply, Ent1, "ballsocket_adv") and WireLib.CanTool(ply, Ent2, "ballsocket_adv") then
