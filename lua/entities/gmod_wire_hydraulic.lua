...
function ENT:Initialize()
	...
	WireLib.CreateOutputs(self, { "Length", "Target Length", "Constant", "Damping" })
	self.TargetLength = 0
...
function ENT:SetLength(value)
	...
	if IsValid(self.rope) then
		self.rope:Fire("SetLength", value, 0)
	end
end
...
function ENT:TriggerInput(iname, value)
	...
	elseif iname == "Constant" then
		...
		timer.Simple(0.1, function()
			if IsValid(self) then -- Needs to be delayed because ent:Fire doesn't update that fast.
				self:UpdateOutputs()
			end
		end)
...
function MakeWireHydraulicController(pl, Pos, Ang, model, MyEntId, const, rope)
	local controller = WireLib.MakeWireEnt(pl, {
