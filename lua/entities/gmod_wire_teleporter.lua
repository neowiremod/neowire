...
function ENT:Initialize()
	...
	self.ClassSpecificActions = {
		gmod_wire_hoverball = function(ent, oldpos, newpos)
			ent:SetZTarget(newpos.z)
		end,
		gmod_toggleablehoverball = function(ent, oldpos, newpos)
			ent:SetTargetZ(newpos.z)
		end,
		gmod_hoverball = function(ent, oldpos, newpos)
			ent.dt.TargetZ = newpos.z
		end,
	}

	self:ShowOutput()
	self.Inputs = WireLib.CreateInputs(
		self,
		{
			"Jump (Activates the teleporter.\nIf nothing happens and an error noise is played, the destination does not have enough room to fit your contraption.)",
			"TargetPos [VECTOR]",
			"X",
			"Y",
			"Z",
...
function ENT:Jump(withangles)
	...
	for _, ent in pairs(ents) do
		...
		elseif ent ~= self then
			-- if the entity is not equal to self
	...
	--------------------------------------------------------------------
	if self.UseSounds then -- Starting sound
...
function ENT:Jump_Part2(withangles)
	...
	for _, ent in pairs(self.Entities) do
		if ent:GetPhysicsObjectCount() > 1 then -- Check for bones
			local tbl = {
				Main = self:WorldToLocal(ent:GetPos()),
			}

			local tbl2 = {
				Main = self:WorldToLocal(ent:GetVelocity() + ent:GetPos()),
			}
	...
	for _, ent in pairs(self.Entities) do
		...
		if withangles then -- Angles
			ent:SetAngles(self:LocalToWorldAngles(self.LocalAng[ent]))
		end
...
function ENT:CheckAllowed(e)
	if e:GetParent():EntIndex() ~= 0 then
		return false
	end
	-- These shouldn't happen, ever, but they're here just to be safe
	local c = e:GetClass()
	if c == "Player" or c:find("npc_") then
		return false
	end
	return true
end
