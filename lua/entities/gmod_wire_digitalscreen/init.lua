...
function ENT:SendPixel()
	if self.Memory[1048575] == 0 then -- why?
		return
	end

	if self.PixelX < 0 then
		return
	end
	if self.PixelY < 0 then
		return
	end
	if self.PixelX >= self.ScreenWidth then
		return
	end
	if self.PixelY >= self.ScreenHeight then
		return
	end
...
local function calcGlobalBW()
	...
	for digi in pairs(globalBandwidthLookup) do
		if not IsValid(digi) then -- this most likely won't trigger due to OnRemove, but just in case
			globalBandwidthLookup[digi] = nil
		end
...
local function removeGlobalBW(e)
	globalBandwidthLookup[e] = nil
end
...
local pixelbits = {
	3, --The compressed pixel formats are in bytes
	1,
	3,
	4,
	1,
...
function ENT:FlushCache(ply)
	...
	if pixelformat < 1 or pixelformat > #pixelbits then
		pixelformat = 1
	end
	...
	if self.ChangedStep > n or (n > self.ScreenWidth * self.ScreenHeight and self.deltaStep * 4 < self.deltaN) then -- reset queue if we've reached the end -- if the queue length keeps growing faster than we can process it, just clear it -- this check is mostly to detect the worst possible case where the user spams single random pixels
		self.ChangedCellRanges = {}
		self.ChangedStep = 1
	end
...
function ENT:WriteCell(Address, value)
	...
	if Address < 1048500 then -- RGB data
		if self.Memory[Address] == value or (value == 0 and self.Memory[Address] == nil) then
			return true
		end
	else
		...
		elseif Address == 1048574 then
			-- Hardware Clear Screen
