...
local function IsWire(entity) --try to find out if the entity is wire
	if WireLib.HasPorts(entity) then
		return true
	end
...
function TOOL:LeftClick(trace)
	...
	if not IsWire(trace.Entity) then
		return
	end
...
if CLIENT then
	function TOOL:DrawHUD()
		if self:GetClientNumber("showports") == 0 then
			return
		end
...
if SERVER then
	...
	local function updateForPlayer(ply, cmps)
		...
		table.Compact(cmps, function(cmp)
			return cmp:IsValid() and IsWire(cmp)
		end)
			...
			for Index, ExplodeLine in ipairs(ExplodeLines) do --break it into multible lines for 1 entry
				if string.Trim(ExplodeLine) ~= "" then
					...
					local TrimLine = {
						...
						OffsetPos = {
							XPos, --move the next text down some for each line
							Line_Count * 14,
						},
					}
