...
if CLIENT then
	...
	TOOL.Information = {
		...
		},
		{
			name = "shift_reload",
			stage = 0,
			text = "Shift + Reload: Remove wirelink and entity outputs",
		},
	}
...
if SERVER then
		...
		if data[1] == true then
			if ent.Outputs then
				...
				for _, v in pairs(ent.Outputs) do
					...
					local num = v.Num
					names[num] = v.Name
					if v.Name == "wirelink" then -- we already have a wirelink output, abort
						return
					end
		...
		if data[1] == true then
			if ent.Outputs then
				...
				for _, v in pairs(ent.Outputs) do
					...
					local num = v.Num
					names[num] = v.Name
					if v.Name == "entity" then -- we already have an entity output, abort
						return
					end
	...
	local function wireAdvUpload(ply, wirings)
		...
		for i = 1, #wirings do
			...
			if IsValid(inputentity) and IsValid(outputentity) and WireLib.CanTool(ply, inputentity, "wire_adv") and WireLib.CanTool(ply, outputentity, "wire_adv") then
...
elseif CLIENT then
	...
	function TOOL:SanitizeUpload() -- Removes any wirings that are no longer valid
		for i = #self.Wiring, 1, -1 do
			...
			local outputentity = wiring[5]
			local outputname = wiring[7]
			if not IsValid(inputentity) or not IsValid(outputentity) or not outputname then -- we don't need to check everything because only these things can possibly be invalid
	...
	function TOOL:CachePorts(ent)
		...
		if self.ShowWirelink then
			if outputs then
				local found = false
				for i = 1, #outputs do
					if outputs[i] and outputs[i][2] == "WIRELINK" then
						found = true
						break
					end
				end
	...
	function TOOL:FindWiring(entity, inputname, inputtype)
		for i = 1, #self.Wiring do
			local wiring = self.Wiring[i]
			if wiring[1] == inputname and wiring[3] == entity and wiring[8] == inputtype then
				return wiring, i
			end
		end
	end
	...
	function TOOL:UpdateTraceForSurface(trace, parent)
		if self:GetClientNumber("stick") == 0 then
			return
		end
		if not WireLib.HasPorts(trace.Entity) then
			return
		end
		local hitParentPos
	...
	function TOOL:LeftClick(trace)
		...
		if IsValid(trace.Entity) then
			if self:GetStage() == 0 then
				...
				if not isTableEmpty(inputs) then
					return
				end
				if alt then -- Select everything
					...
					if not inputs[self.CurrentWireIndex] then -- Can happen if theres no inputs, only outputs
						return
					end
			...
			elseif self:GetStage() == 1 then
				...
				if not isTableEmpty(outputs) then
					return
				end
	...
	function TOOL:Scroll(trace, dir)
		...
		if IsValid(ent) then
			...
			if not isTableEmpty(inputs) and not isTableEmpty(outputs) then
				return
			end
	...
	function TOOL:IsHighlighted(name, tbl, ent, idx)
		...
		elseif name == "Inputs" and self:GetStage() == 0 then
			...
			if wiring then -- Highlight with a different color
				return true, true
			end
	...
	local fontData = {
		font = "Trebuchet MS", -- 24 and 18 are stock
	}
	...
	function TOOL:BeginRenderingCurrentWire()
		if self.IsRenderingCurrentWire then
			return
		end
	...
	function TOOL:IsMatch(inputname, inputtype, outputname, outputtype, checkcreated)
		if checkcreated then
			return (outputname == inputname and outputtype == inputtype)
				or (inputtype == "WIRELINK" and (outputname == "wirelink" or outputname == "Create Wirelink") and outputtype == "WIRELINK")
				or (inputtype == "ENTITY" and (outputname == "entity" or outputname == "Create Entity") and outputtype == "ENTITY")
		else
			return outputname == inputname and outputtype == inputtype
		end
	end
