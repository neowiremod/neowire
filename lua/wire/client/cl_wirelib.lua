...
local function getmat(mat)
	if not mats_cache[mat] then --Just not to create a material every frame
		mats_cache[mat] = Material(mat)
	end
	return mats_cache[mat]
end

function Wire_Render(ent)
	if Wire_DisableWireRender ~= 0 then --We shouldn't render anything
		return
	end
...
function Wire_DrawTracerBeam(ent, beam_num, hilight, beam_length)
	...
	if ent.GetTarget and (ent:GetTarget().X ~= 0 or ent:GetTarget().Y ~= 0 or ent:GetTarget().Z ~= 0) then
		...
		if trace.endpos[1] ~= trace.endpos[1] then
			trace.endpos = pos + Vector(ent:GetBeamLength(), 0, 0)
		end
...
if old_renderhalos ~= nil then
	hook.Add("PostDrawEffects", "RenderHalos", function()
		if hook.Run("ShouldDrawHalos") == false then
			return
		end
		old_renderhalos()
	end)
