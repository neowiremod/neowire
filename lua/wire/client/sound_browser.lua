...
local function GetFileInfos(strfile)
	if not isstring(strfile) or strfile == "" then
		return
	end
...
local function GetFileSource(strFile) -- we have to do this because util.RelativePathToFull_Menu is restricted to menu state :( --strFile is sound/filepath.wav, not filepath.wav
	...
	if not string.match(strFile, "/") then --GetSoundScriptSource(strFile) --alright this literally has no use whatsoever
		return
	end

	-- remove "special" characters from sound files, used in soundscripts.
	if strFile:sub(1, 5) == "sound" then
		strFile = string.gsub(strFile, "^sound/%W*", "sound/")
	end
	...
	if file.Exists(strFile, "DOWNLOAD") then
		return "Server Download", "download"
	end
	if file.Exists(strFile, "BSP") then
		return "Current Map (" .. game.GetMap() .. ")", "bspfile"
	end
...
local function GenerateInfoTree(strfile, backnode, count)
	if oldstrfile == strfile and strfile then
		return
	end
	oldstrfile = strfile
	...
	if IsFile then
		...
		do
			...
			subnode = node:AddNode(
				SoundData[index][3] == "game" and SoundData[index][4] or SoundData[index][2],
				SoundData[index][3] == "game" and "games/16/" .. SoundData[index][2] .. ".png"
					or SoundData[index][3] == "legacy" and "icon16/folder_brick.png"
					or SoundData[index][3] == "workshop" and "games/16/all.png"
					or SoundData[index][3] == "download" and "icon16/transmit.png"
					or SoundData[index][3] == "bspfile" and "icon16/world.png"
					or "icon16/folder_link.png"
			)
		end
		...
		do
			...
			node.Expander.DoClick = function(self)
				if not IsValid(SoundInfoTree) then
					return
				end
...
local function CreateSoundBrowser(path, se)
	...
	if IsValid(SoundBrowserPanel) then
		SoundBrowserPanel:Remove()
	end
	if IsValid(TabFileBrowser) then
		TabFileBrowser:Remove()
	end
	if IsValid(TabSoundPropertyList) then
		TabSoundPropertyList:Remove()
	end
	if IsValid(TabFavourites) then
		TabFavourites:Remove()
	end
	if IsValid(SoundInfoTree) then
		SoundInfoTree:Remove()
	end
	if IsValid(SoundInfoTreeRoot) then
		SoundInfoTreeRoot:Remove()
	end
	...
	SoundInfoTree.DoClick = function(parent, node)
		...
		if (Clicktime - oldClicktime) > 0.3 then
			oldClicktime = Clicktime
			return
		end

		oldClicktime = Clicktime
		if not node.IsSoundNode then
			return
		end
	...
	TabFileBrowser.LineData = function(self, id, strfile, ...)
		if #strfile > max_char_count then -- skip and hide to long filenames.
			return nil, true
		end
	...
	TabFileBrowser.OnLineAdded = function(self, id, line, strfile, ...) end
	...
	SoundBrowserPanel.OnClose = function()
		-- Set effects back and mute when closing.
		nSoundVolume = 1
		nSoundPitch = 100
...
local function OpenSoundBrowser(pl, cmd, args)
	...
	if not IsValid(TabFileBrowser) then
		return
	end
