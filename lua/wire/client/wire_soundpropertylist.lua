...
local function IsInString(strSource, strPattern)
	...
	if string.find(strSource, strPattern, 0, true) then
		return true
	end
	return false
end
...
local function GenerateList(self, strPattern)
	...
	WireLib.Timedpairs(self.TimedpairsName, soundtable, self.m_nListSpeed, function(k, v, self)
		...
		if not IsValid(self.SoundProperties) then
			return false
		end
		if not IsValid(self.SearchProgress) then
			return false
		end
		...
		if not IsInString(v, strPattern) then
			return
		end
		self:AddItem(k, v)
...
function PANEL:PerformLayout()
	if not self.SearchProgress:IsVisible() then
		return
	end
	self.SearchProgressLabel:SizeToContents()
	self.SearchProgressLabel:Center()
