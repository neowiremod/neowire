...
local function AsForOverride(self, func, filename, ...)
	...
	if filename == self.m_strList then
		func(self, filename, ...)
		return
	end
...
function PANEL:UpdateListNameLabel()
	if not IsValid(self.ListNameLabel) then
		return
	end
...
function PANEL:AddItem(...)
	...
	if not isstring(item) or item == "" then
		return
	end
	if self.TabfileCount > self.m_nMaxItems then
		return
	end
	if #item > max_char_count then
		return
	end
	if self.Tabfile[item] then
		return
	end
	local itemargs = {}
	local i = 0
...
function PANEL:RemoveItem(item)
	...
	if not self.Tabfile[item] then
		return
	end
	if not self.Files.Lines then
		return
	end
...
function PANEL:SaveList(strfile)
	...
	AsForOverride(self, function(self, strfile)
		...
		if not filedata then
			Derma_Query("File could not be saved!", "Error!", "OK")
			return
		end
