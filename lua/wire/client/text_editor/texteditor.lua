...
function EDITOR:Init()
	...
	self.TextEntry.OnKeyCodeTyped = function(self, code)
		return self.Parent:_OnKeyCodeTyped(code)
	end
...
function EDITOR:DoAction(name, ...)
	...
	if not f then
		f = WireTextEditor.Modes.Default[name]
	end
	if f then
		return f(self, ...)
	end
	...
	if code == MOUSE_LEFT then
		...
		if (CurTime() - self.LastClick) < 1 and self.tmp and cursor[1] == self.Caret[1] and cursor[2] == self.Caret[2] then
			...
			if wire_expression2_editor_highlight_on_double_click:GetBool() then
				...
				if all_finds then
					all_finds[0] = {
						1, -- Set [0] so the [i-1]'s don't fail on the first iteration
						1,
					}
					...
					for i = 1, #all_finds do
						...
						if caretstart[1] ~= self.Start[1] or caretstart[2] ~= self.Start[2] or caretstop[1] ~= self.Caret[1] or caretstop[2] ~= self.Caret[2] then
...
function EDITOR:HighlightLine(line, r, g, b, a)
	if not self.HighlightedLines then
...
function EDITOR:ClearHighlightedLines()
...
function EDITOR:ClearHighlightedAreas()
	self.HighlightedAreas = nil
end
...
function EDITOR:OnTextChanged() end
...
function EDITOR:OnMouseWheeled(delta)
	if self.AC_Panel and self.AC_Panel:IsVisible() then
		...
		if mode == AC_STYLE_SCROLLER or mode == AC_STYLE_SCROLLER_ENTER then
			...
			if self.AC_Panel.Selected > #self.AC_Suggestions then
				self.AC_Panel.Selected = 1
			end
			if self.AC_Panel.Selected < 1 then
				self.AC_Panel.Selected = #self.AC_Suggestions
			end
...
function EDITOR:OnShortcut() end
...
function EDITOR:Replace(str, replacewith)
	if str == "" or str == replacewith then
		return
	end
	local use_patterns = wire_expression2_editor_find_use_patterns:GetBool()
...
function EDITOR:CreateFindWindow()
	...
	do
		...
		use_patterns.Button.SetValue = function(pnl, b)
			if wire_expression2_editor_find_whole_word_only:GetBool() then
				return
			end
			old(pnl, b)
		end
...
function EDITOR:OpenFindWindow(mode)
	...
	if mode == "find" then
		if selection and selection ~= "" then
			self.FindWindow.FindTab.Entry:SetText(selection)
		end
	...
	elseif mode == "find and replace" then
		if selection and selection ~= "" then
			self.FindWindow.ReplaceTab.Entry:SetText(selection)
		end
...
function EDITOR:PreviousTab()
	...
	if currentTab < 1 then
		currentTab = currentTab + parent:GetNumTabs()
	end
...
function EDITOR:NextTab()
	...
	if currentTab > numTabs then
		currentTab = currentTab - numTabs
	end
	parent:SetActiveTabIndex(currentTab)
end
	...
	if control then
		...
		if code == KEY_ENTER then
			...
			if mode == AC_STYLE_ECLIPSE and self.AC_HasSuggestions and self.AC_Suggestions[1] and self.AC_Panel and self.AC_Panel:IsVisible() then
				if self:AC_Use(self.AC_Suggestions[1]) then
					return
				end
			end
...
tbl[1] = function(self)
	if self.ac_event or self.ac_directive_line or self.ac_function_type then
		return
	end
	...
	if word:sub(1, 1):lower() ~= word:sub(1, 1) then
		return
	end
...
tbl[2] = function(self)
	if self.ac_event or self.ac_directive_line then
		return
	end
...
local function GetTableForFunction(udf)
	return {
		...
		replacement = function(t, editor)
			...
			local has_bracket = editor:GetArea({
				wordend, -- If there already is a bracket, we don't want to add more of them.
				{ wordend[1], wordend[2] + 1 },
			}) == "("
...
local function FindVariables(self, word)
	...
	local suggested = {}
	local suggestions = {}
	if not self.AC_Directives or not self.AC_Variables then
...
tbl.RunOnCheck = function(self)
	...
	elseif not self:IsVarLine() and self.AC_WasVarLine then
		-- If the user ISN'T editing a var line, and they WERE editing a var line before this..
...
function EDITOR:AC_Check(notimer)
	...
	if not self.AC_AutoCompletion then -- Default to E2 autocompletion
		self:AC_NewAutoCompletion(tbl)
	end
...
tbl[5] = function(self)
	if not self.ac_event or self.ac_directive_line then
		return
	end
...
function EDITOR:AC_CreatePanel()
	...
	panel.Think = function(pnl, code)
		if not self.AC_HasSuggestions or not self.AC_Panel_Visible then
			return
		end
		local mode = wire_expression2_autocomplete_controlstyle:GetInt()
		...
		elseif mode == AC_STYLE_VISUALCSHARP or mode == AC_STYLE_ATOM then
			if input.IsKeyDown(KEY_TAB) or input.IsKeyDown(KEY_ENTER) or input.IsKeyDown(KEY_SPACE) then -- Use
				self:AC_SetVisible(false)
				self:AC_Use(self.AC_Suggestions[pnl.Selected])
			elseif input.IsKeyDown(KEY_DOWN) and not pnl.AlreadySelected then
				-- Select
				pnl.Selected = pnl.Selected + 1 -- Scroll down
				if pnl.Selected > #self.AC_Suggestions then
					pnl.Selected = 1
				...
				self:AC_FillInfoList(self.AC_Suggestions[pnl.Selected]) -- Fill the info list
				pnl.AlreadySelected = true -- To keep it from scrolling a thousand times a second
			elseif input.IsKeyDown(KEY_UP) and not pnl.AlreadySelected then
				-- Select
				pnl.Selected = pnl.Selected - 1 -- Scroll up
				if pnl.Selected < 1 then
					pnl.Selected = #self.AC_Suggestions
...
function EDITOR:AC_FillInfoList(suggestion)
	...
	local maxh = 0
	local others
	if suggestion.others then
		others = suggestion:others(self)
	end
	...
	if panel.curh < texth + 4 then
		panel:SetTall(texth + 6)
	else
		panel:SetTall(panel.curh)
	end

	if maxh + texth > panel:GetTall() then
		maxw = maxw + 25
	end
...
function EDITOR:AC_SetVisible(bool)
	if self.AC_Panel_Visible == bool or not self.AC_Panel then
		return
	end
...
function EDITOR:SkipPattern(pattern)
	...
	if startpos ~= self.position then
		return nil
	end
