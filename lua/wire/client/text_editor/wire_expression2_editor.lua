...
function Editor:OnMousePressed(mousecode)
	if mousecode ~= 107 then -- do nothing if mouseclick is other than left-click
		return
	end
...
function Editor:getMode()
	...
	if ix < 0 or ix > w or iy < 0 or iy > h then -- if the mouse is outside the box
		return
	end
...
function Editor:GetEditorMode()
	return self.EditorMode
end

local old
function Editor:FixTabFadeTime()
	if old ~= nil then -- It's already being fixed
		return
	end
...
function Editor:InitComponents()
	...
	self.C.SaE.DoClick = function(button)
	...
	self.C.Val.OnQuickFix = function(panel, issue)
		...
		for _, fix in ipairs(issue.quick_fix) do
			local trace = fix.at
			editor:SetArea({ { trace.start_line, trace.start_col }, { trace.end_line, trace.end_col } }, fix.replace)
		end
...
function Editor:AutoSave()
	...
	if self.savebuffer == buffer or buffer == defaultcode or buffer == "" then
		return
	end
...
function Editor:AddControlPanelTab(label, icon, tooltip)
	...
	function ret.Tab.OnMousePressed(...)
		timer.Simple(0.1, function()
			frame:ResizeAll() -- timers solve everything
		end)
...
function Editor:InitControlPanel(frame)
	...
	blockCommentModes["New (alt 1)"] = { 0, [[Current mode:
#[
Text here
Text here
]#]] }
	blockCommentModes["New (alt 2)"] = { 1, [[Current mode:
#[Text here
Text here]# ]] }
	blockCommentModes["Old"] = { 2, [[Current mode:
#Text here
#Text here]] }
	...
	local painted = 0
	local opened = false
	dlist2.Paint = function()
		painted = SysTime() + 0.05
	end
	timer.Create("wire_expression2_ops_sync_check", 0, 0, function()
		if painted > SysTime() and not opened then
			opened = true
			if Editor.ops_sync_subscribe:GetBool() then
				RunConsoleCommand("wire_expression_ops_sync", "1")
			end
		...
		for _, v in ipairs(E2s) do
			...
			if IsValid(ply) and ply == LocalPlayer() or showall then
				...
				if LocalPlayer():IsAdmin() then
					...
					function label:Think()
						...
						if data then
							...
							label:SetText(string.format("Name: %s\nEntity ID: '%d'\n%i ops, %i%% %s\ncpu time: %ius\n%s", name, idx, prfbench, prfbench / e2_softquota * 100, hardtext, timebench * 1000000, ownerStr))
						end
					end
				end
				...
				do
					...
					btn:SetSize(57, 18)
					timer.Simple(0, function()
						btn:SetPos(panel:GetWide() - btn:GetWide() * 2 - 6, 4)
					end)
				...
				do
					...
					btn:SetSize(57, 18)
					timer.Simple(0, function()
						btn:SetPos(panel:GetWide() - btn:GetWide() - 4, 4)
					end)
					btn.DoClick = function(pnl)
				...
				do
					...
					btn:SetSize(75, 18)
					timer.Simple(0, function()
						btn:SetPos(panel:GetWide() - btn:GetWide() - 4, 24)
					end)
					btn.DoClick = function(pnl)
					...
					btn2:SetSize(39, 18)
					timer.Simple(0, function()
						btn2:SetPos(panel:GetWide() - btn2:GetWide() - btn:GetWide() - 6, 24)
					end)
	...
	UpdateList2.DoClick = function(pnl)
		UpdateList:DoClick(true)
	end
end
...
function Editor:OpenOldTabs()
	if not file.Exists(self.Location .. "/_tabs_.txt", "DATA") then
		return
	end
	-- Read file
...
function Editor:Open(Line, code, forcenewtab)
	if self:IsVisible() and not Line and not code then
		self:Close()
	end
...
function Editor:SaveFile(Line, close, SaveAs)
	...
	if close and self.chip then
		if not self:Validate(true) then
			return
		end
	...
	if not Line or SaveAs or Line == self.Location .. "/" .. ".txt" then
		...
		Derma_StringRequestNoBlur("Save to New File", "", (str ~= nil and str .. "/" or "") .. self.savefilefn, function(strTextOut)
			...
			if file.Exists(save_location, "DATA") then
				Derma_QueryNoBlur("The file '" .. strTextOut .. "' already exists. Do you want to overwrite it?", "File already exists", "Yes", function()
	...
	if f then
		...
		timer.Simple(0, function()
			panel.SetText(panel, "   Saved as " .. Line)
		end)
