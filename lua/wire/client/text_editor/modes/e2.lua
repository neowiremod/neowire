...
local colors = {
	["directive"] = {
		Color(240, 240, 160), -- yellow
		false,
	},
	["number"] = {
		Color(240, 160, 160), -- light red
		false,
	},
	["function"] = {
		Color(160, 160, 240), -- blue
		false,
	},
	["notfound"] = {
		Color(240, 96, 96), -- dark red
		false,
	},
	["variable"] = {
		Color(160, 240, 160), -- light green
		false,
	},
	["string"] = {
	...
	["keyword"] = {
		Color(160, 240, 240), -- turquoise
		false,
	},
	["operator"] = {
		Color(224, 224, 224), -- white
		false,
	},
	["comment"] = {
		Color(128, 128, 128), -- grey
	...
	["ppcommand"] = {
		Color(240, 96, 240), -- purple
		false,
	},
	["ppcommandargs"] = {
		Color(128, 128, 128), -- same as comment
		false,
	},
	["typename"] = {
		Color(240, 160, 96), -- orange
		false,
	},
	["constant"] = {
		Color(240, 160, 240), -- pink
		false,
	},
	["userfunction"] = {
		Color(102, 122, 102), -- dark grayish-green
		false,
	},
	["eventname"] = {
		Color(74, 194, 116), -- green
		false,
	},
	["background"] = {
		Color(32, 32, 32), -- dark-grey
		false,
	},
}
...
function EDITOR:SyntaxColorLine(row)
	...
	elseif self:NextPattern("^@[^ ]*") then
		...
		if not highlightmode then
			return { { "@", colors.directive }, { self.line:sub(2), colors.notfound } }
		end
		-- check for plain text directives
		if highlightmode == 0 then
			return { { self.line, colors.directive } }
		end
	...
	if found then
		...
		if self:NextPattern("[a-z][a-zA-Z0-9]*%s%s*[a-z][a-zA-Z0-9]*:[a-z][a-zA-Z0-9_]*") then -- Everything specified (returntype typeindex:funcname)
			...
			self.tokendata = ""
		elseif self:NextPattern("[a-z][a-zA-Z0-9]*%s%s*[a-z][a-zA-Z0-9_]*") then
			-- returntype funcname
		...
		if self:NextPattern("%(") then -- We found a bracket
			...
			while self.character and self.character ~= ")" do
				...
				if dots then
					addToken("operator", dots)
				end
				local invalidInput = self:SkipPattern("[^A-Z:%[_]*")
				if invalidInput then
					addToken("notfound", invalidInput)
				end
				if self:NextPattern("%[") then -- Found a [
					-- Color the bracket
					addToken("operator", self.tokendata)
					self.tokendata = ""
					while acceptIdent(self) do
