...
function WireToolObj:LeftClick(trace)
	if not trace.HitPos or trace.Entity:IsPlayer() or trace.Entity:IsNPC() or (SERVER and not util.IsValidPhysicsObject(trace.Entity, trace.PhysicsBone)) then
		return false
	end
	if self.NoLeftOnClass and trace.HitNonWorld and (trace.Entity:GetClass() == self.WireClass or NoGhostOn(self, trace)) then
		return false
	end
	if CLIENT then
		return true
...
if SERVER then
	...
	function WireToolObj:LeftClick_Make(trace, ply)
		...
		if not self:CheckMaxLimit() or not self:CheckValidModel(model) then
			return false
		end
	...
	function WireToolObj:MakeEnt(ply, model, Ang, trace)
		...
		if ent and ent.RestoreNetworkVars then
	...
	function WireToolObj:LeftClick_PostMake(ent, ply, trace)
		...
		if ent == nil or ent == false or not ent:IsValid() then
			return false
		end
		-- Parenting
...
function WireToolObj:GetModel()
	...
	if self.ClientConVar.modelsize then
		...
		if modelsize ~= "" then
			...
			if self:CheckValidModel(model) then
				return model
			end
...
function WireToolHelpers.GetActiveTOOL(mode, ply)
	...
	if not IsValid(activeWep) or activeWep:GetClass() ~= "gmod_tool" or activeWep.Mode ~= mode then
		return
	end
...
function WireToolHelpers.SetupSingleplayerClickHacks(TOOL) -- empty stub outside of Singleplayer
end
...
if SERVER then
	...
	net.Receive("WireLinkedEntsRequest", function(netlen, ply)
		...
		if not controller.WireLinkedEnts then
			return
		end
		if not controller.WireLinkedEnts.Marks then
			return
		end
		if controller.WireLinkedEnts.LastUpdated < lastUpdated then
			return
		end
