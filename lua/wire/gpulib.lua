...
if CLIENT then
	...
	local function GPULib_MemorySync(um)
		...
		if not GPU:IsValid() then
			return
		end
		if not writeHandler[GPUIdx] then
			return
		end
...
elseif SERVER then
	...
	local function getSize(value)
		if (value >= -128) and (value <= 127) and (math.floor(value) == value) then
			return 1, false
		end
		if (value >= -32768) and (value <= 32767) and (math.floor(value) == value) then
			return 2, false
		end
		if (value >= -2147483648) and (value <= 2147483647) and (math.floor(value) == value) then
			return 4, false
		end
		return 4, true
	end
