...
GateActions["string_sub"] = {
	...
	output = function(gate, A, B, C)
		...
		if not B then -- defaults to start of string
			B = 1
		end

		if not C then -- defaults to end of string
			C = -1
		end
...
GateActions["string_concat"] = {
	...
	output = function(gate, A, B, C, D, E, F, G, H)
		if (A and #A or 0) + (B and #B or 0) + (C and #C or 0) + (D and #D or 0) + (E and #E or 0) + (F and #F or 0) + (G and #G or 0) + (H and #H or 0) > MAX_LEN then
			return false
		end
