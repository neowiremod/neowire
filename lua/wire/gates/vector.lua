...
GateActions["vector_add"] = {
	...
	label = function(Out, ...)
		...
		return string.format("%s = (%d,%d,%d)", string.sub(tip, 3), Out.x, Out.y, Out.z)
	end,
}
...
GateActions["vector_mux"] = {
	...
	label = function(Out, Sel, ...)
		return string.format("Select: %s  Out: (%d,%d,%d)", Sel, Out.x, Out.y, Out.z)
	end,
}
...
GateActions["vector_dmx"] = {
	...
	output = function(gate, Sel, In)
		local Out = { Vector(0, 0, 0), Vector(0, 0, 0), Vector(0, 0, 0), Vector(0, 0, 0), Vector(0, 0, 0), Vector(0, 0, 0), Vector(0, 0, 0), Vector(0, 0, 0) }
		Sel = math.floor(Sel)
		if Sel > 0 and Sel <= 8 then
			Out[Sel] = In
		end
	...
	label = function(Out, Sel, In)
		...
		return string.format("Select: %s, In: (%d,%d,%d)", Sel, In.x, In.y, In.z)
	end,
}
...
GateActions["vector_latch"] = {
	...
	label = function(Out, In, Clk)
		return string.format("Latch Data: %s  Clock: %s  Out: (%d,%d,%d)", In, Clk, Out.x, Out.y, Out.z)
	end,
}
