...
function WireLib.TriggerInput(ent, name, value, ...)
	if not IsValid(ent) or not HasPorts(ent) or not ent.Inputs then
		return
	end
...
WireLib.DT = {
	...
	VECTOR2 = {
		...
		Validator = function(v2)
			return istable(v2) and isnumber(v2[1]) and isnumber(v2[2])
		end,
	},
	VECTOR4 = {
		Zero = function()
		...
		Validator = function(v4)
			return istable(v4) and isnumber(v4[1]) and isnumber(v4[2]) and isnumber(v4[3]) and isnumber(v4[4])
		end,
	},
	ANGLE = {
		Zero = Angle,
	...
	TABLE = {
		...
		Validator = function(t)
			return istable(t) and istable(t.n) and istable(t.ntypes) and istable(t.s) and istable(t.stypes) and isnumber(t.size)
		end,
	},
	BIDIRTABLE = {
		Zero = function()
...
function WireLib.RetypeInputs(ent, iname, itype, descs)
	if not HasPorts(ent) then
		return
	end
	local ent_ports = ent.Inputs
	if (not ent_ports[iname]) or not itype then
		return
	end
...
function WireLib.RetypeOutputs(ent, oname, otype, descs)
	...
	if (not ent_ports[oname]) or not otype then
		return
	end
...
function WireLib.Remove(ent, DontUnList)
	...
	if DontUnList then -- Set DontUnList to true if you want to remove ent from the list manually.
		return
	end
...
function WireLib.TriggerOutput(ent, oname, value, iter)
	...
	if value ~= output.Value or output.Type == "ARRAY" or output.Type == "TABLE" or (output.Type == "ENTITY" and not rawequal(value, output.Value)) then --[[Covers the NULL==NULL case]]
...
function WireLib.Link_Start(idx, ent, pos, iname, material, color, width)
	...
	if not ent.Inputs or not ent.Inputs[iname] then
		return
	end
	local input = ent.Inputs[iname]
...
function WireLib.Link_Node(idx, ent, pos)
	if not CurLink[idx] then
		return
	end
	if not IsValid(CurLink[idx].Dst) then
		return
	end
	if not IsValid(ent) then -- its the world, give up
		return
	end
...
function WireLib.Link_End(idx, ent, pos, oname, pl)
	...
	if not HasPorts(CurLink[idx].Dst) then
		return
	end
	if not IsValid(ent) then
		return
...
function WireLib.WireAll(ply, ient, oent, ipos, opos, material, color, width)
	if not IsValid(ient) or not IsValid(oent) or not ient.Inputs or not oent.Outputs then
		return false
	end
...
do
	...
	function OutputIterator:Process()
		if self.Processing then -- should not occur
			return
		end
...
function WireLib.Weld(ent, traceEntity, tracePhysicsBone, DOR, collision, AllowWorldWeld)
	if not ent or not traceEntity or traceEntity:IsNPC() or traceEntity:IsPlayer() then
		return
	end
...
function WireLib.ApplyDupeInfo(ply, ent, info, GetEntByID)
	if info.extended and not ent.extended then
		WireLib.CreateWirelinkOutput(ply, ent, {
			true, -- old dupe compatibility; use the new function
		})
	end

	local idx = 0
	if IsValid(ply) then -- Map Save loading does not have a ply
		idx = ply:UniqueID()
	end
...
hook.Add("InitPostEntity", "antiantinoclip", function()
	...
	function ENT:StartTouch(...)
		...
		if phys:IsValid() and phys:GetAngleVelocity():Length() > 20 then
			return
		end
...
function WireLib.CanModel(player, model, skin)
	...
	if skin ~= nil then
		...
		if skin < 0 or (count and skin >= count) then
			return false
		end
	end

	if IsValid(player) and player:IsPlayer() and not hook.Run("PlayerSpawnObject", player, model, skin) then
		return false
	end
	return true
end
...
function WireLib.MakeWireEnt(pl, Data, ...)
	...
	if IsValid(pl) and not pl:CheckLimit(Data.Class:sub(6) .. "s") then
		return false
	end
	if Data.Model and not WireLib.CanModel(pl, Data.Model, Data.Skin) then
		return false
	end
...
function WireLib.CheckRegex(data, pattern)
	local limits = {
		[0] = 50000000, -- Worst case is about 200ms
		15000,
		500,
		150,
		70,
	...
	-- Find extenders
	local n = 0
	for i in string.gmatch(stripped, "[%+%-%*]") do
		n = n + 1
	end

	local msg
	if n <= #limits then
		if #data * (#stripped + nrepl - n + nrepl2) > limits[n] then
			msg = n .. " ext search length too long (" .. limits[n] .. " max)"
		else
			return
		end
	else
...
function WireLib.IsValidMaterial(material)
	...
	if material_blacklist[path] then
		return ""
	end
	return material
end
