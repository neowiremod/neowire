	...
	TOOL.Information = {
		{
			name = "left_0",
			stage = 0,
			text = "Choose the wheel's axis",
		},
		{
			name = "left_1",
			stage = 1,
			text = "Choose the base's axis",
		},
		{
			name = "left_2",
			stage = 2,
...
function TOOL:GetConVars()
	return self:GetClientNumber("torque"), self:GetClientNumber("friction"), self:GetClientNumber("nocollide"), self:GetClientNumber("forcelimit"), self:GetClientNumber("offset")
end
...
function TOOL:LeftClick_Update(trace)
	...
	if IsValid(const) then
		...
		if IsValid(trace.Entity.axis) then
			trace.Entity.axis:DontDeleteOnRemove(trace.Entity)
		end
		-- Get constraint info
		...
		local const, axis = MakeWireMotor(self:GetOwner(), tbl.Ent1, tbl.Ent2, tbl.Bone1, tbl.Bone2, tbl.LPos1, tbl.LPos2, friction, torque, nocollide, forcelimit)
...
function TOOL:LeftClick(trace)
	...
	if iNum == 0 and not IsValid(trace.Entity) then
		return
	end
	-- Don't allow us to choose the same object
	if iNum == 1 and trace.Entity == self:GetEnt(1) then
		return
	end
	-- Get client's CVars
