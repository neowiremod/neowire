	...
	TOOL.Information = {
		{
			name = "left_0",
			stage = 0,
			text = "Create Entity Marker/Display Link Info",
		},
		{
			name = "right_0",
			stage = 0,
			text = "Link Entity Marker",
		},
		{
			name = "reload_0",
			stage = 0,
			text = "Unlink Entity Marker",
		},
		{
			name = "right_1",
			stage = 1,
			text = "Now select the entity to link to",
		},
	}
end
...
function TOOL:DrawHUD()
	...
	if not IsValid(mark) or not IsValid(marker) then
		return
	end
