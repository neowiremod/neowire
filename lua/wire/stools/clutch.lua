	...
	TOOL.Information = {
		{
			name = "left_0",
			stage = 0,
			text = "Place/Select a clutch controller",
		},
		{
			name = "right_0",
			stage = 0,
			text = "Select an entity to apply the clutch to",
		},
		{
			name = "reload_0",
			stage = 0,
			text = "Remove clutch from entity/deselect controller",
		},
		{
			name = "right_1",
			stage = 1,
			text = "Right click on the second entity you want the clutch to apply to",
		},
	}
end
	...
	function TOOL:DrawHUD()
		...
		if numconstraints_0 ~= 0 then
			...
			for k, v in pairs(Linked_Ents) do
				...
				if IsValid1 then
					pos1 = v.Ent1:GetPos():ToScreen()
				end
				if IsValid2 then
					pos2 = v.Ent2:GetPos():ToScreen()
				end
...
function TOOL:RightClick(trace)
	...
	if (not IsValid(trace.Entity) and not trace.Entity:IsWorld()) or trace.Entity:IsPlayer() then
		return
	end
...
function TOOL.BuildCPanel(panel)
	panel:AddControl("Header", {
		Text = "#Tool.wire_clutch.name",
		Description = "#Tool.wire_clutch.desc",
	})
