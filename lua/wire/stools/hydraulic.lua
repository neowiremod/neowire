	...
	TOOL.Information = {
		{
			name = "left_0",
			stage = 0,
			text = "Place hydraulic",
		},
		{
			name = "right_0",
			stage = 0,
			text = "Place hydraulic along the hit normal",
		},
		{
			name = "left_1",
			stage = 1,
			text = "Choose the second point",
		},
		{
			name = "left_2",
			stage = 2,
			text = "Place the controller",
		},
	}
end
...
function TOOL:GetConVars()
	return self:GetClientInfo("material") or "cable/rope", self:GetClientNumber("width", 3), self:GetClientNumber("speed", 16), self:GetClientNumber("fixed", 0), self:GetClientNumber("stretchonly", 0) ~= 0
end
...
function TOOL:LeftClick_Update(trace)
	...
	if IsValid(const) then
		...
		if IsValid(trace.Entity.rope) then
			trace.Entity.rope:DontDeleteOnRemove(trace.Entity)
		end
		-- Get constraint info
		...
		local const, rope = MakeWireHydraulic(self:GetOwner(), tbl.Ent1, tbl.Ent2, tbl.Bone1, tbl.Bone2, tbl.LPos1, tbl.LPos2, width, material, speed, fixed, stretchonly)
...
function TOOL:LeftClick(trace)
	if not trace.Hit or (trace.Entity:IsValid() and trace.Entity:IsPlayer()) then
		return
	end
	if SERVER and not util.IsValidPhysicsObject(trace.Entity, trace.PhysicsBone) then
		return false
	end
...
function TOOL:RightClick(trace)
	...
	if not WireLib.CanTool(self:GetOwner(), trace2.Entity, "wire_hydraulic") then
		return false
	end
...
function TOOL.BuildCPanel(panel)
	...
	panel:AddControl("RopeMaterial", {
		Label = "#Tool.wire_hydraulic.material",
		convar = "wire_hydraulic_material",
	})
end
