...
if SERVER then
	...
	local function SpawnEnt(ply, Pos, Ang, model, class)
		if IsValid(ply) and (not ply:CheckLimit("wire_egps")) then
			return false
		end
		if not ply then -- For Garry's Map Saver
			ply = game.GetWorld()
		end

		if model and not WireLib.CanModel(ply, model) then
			return false
		end
	...
	function TOOL:LeftClick(trace)
		...
		if not ply:CheckLimit("wire_egps") then
			return false
		end
		local ent
...
if CLIENT then
	...
	function TOOL:LeftClick(trace)
		return not trace.Entity or (trace.Entity and not trace.Entity:IsPlayer())
	end
	...
	function TOOL.BuildCPanel(panel)
		...
		panel:AddControl("Checkbox", {
			Label = "#Tool_wire_egp_weld",
			Command = "wire_egp_weld",
		})

		panel:AddControl("Checkbox", {
			Label = "#Tool_wire_egp_weldworld",
			Command = "wire_egp_weldworld",
		})

		panel:AddControl("Checkbox", {
			Label = "#Tool_wire_egp_freeze",
			Command = "wire_egp_freeze",
		})

		panel:AddControl("Checkbox", {
			Label = "#Tool_wire_egp_translucent",
			Command = "wire_egp_translucent",
		})

		panel:AddControl("Checkbox", {
			Label = "#Tool_wire_egp_drawemitters",
			Command = "wire_egp_drawemitters",
		})

		panel:AddControl("Checkbox", {
			Label = "#Tool_wire_egp_emitter_usert",
			Command = "wire_egp_emitter_usert",
		})
