...
if SERVER then
	...
	function TOOL:RightClick(trace)
		if not IsValid(trace.Entity) or trace.Entity:GetClass() ~= "gmod_wire_value" then
			return false
		end
...
if CLIENT then
	...
	local typeGuessing = {
		-- we're not checking angle because it's indistinguishable from vectors
	...
	local function loadPreset(data)
		...
		if isnumber(next(data)) then
			...
			for i = 1, table.Count(data), 2 do
				table.insert(values, {
					DataType = data[tostring(i)],
					Value = data[tostring(i + 1)],
				})
			end
		end
