...
if CLIENT then
	...
	TOOL.Information = {
		{
			name = "left",
			text = "Click a prop to turn it into a " .. TOOL.Name,
		},
	}
end
...
function TOOL:LeftClick(trace)
	...
	if ent:GetClass() ~= "prop_physics" and ent:GetClass() ~= "gmod_wire_spawner" then
		return false
	end
	if CLIENT then
		return true
	...
	if not self:GetSWEP():CheckLimit("wire_spawners") then
		return false
	end
	...
	local wire_spawner = WireLib.MakeWireEnt(pl, {
		Class = self.WireClass,
		Pos = Pos,
		Angle = Ang,
		Model = model,
	}, delay, undo_delay, spawn_effect, mat, c.r, c.g, c.b, c.a, skin)

	if not wire_spawner:IsValid() then
		return
	end
...
function TOOL:Think() -- Disable ghost
end
