...
function TOOL:GetModel()
	...
	if not util.IsValidModel(model) or not util.IsValidProp(model) or not SocketData[model] then
		return "models/props_lab/tpplugholder_single.mdl"
	end
	return model
end
...
function TOOL:RightClick(trace)
	...
	local plug = WireLib.MakeWireEnt(ply, {
		Class = "gmod_wire_plug",
		Pos = trace.HitPos,
		Angle = self:GetAngle(trace),
		Model = plugmodel,
	}, self:GetClientNumber("array") ~= 0)

	if not IsValid(plug) then
		return false
	end
