...
if SERVER then
	...
	local function InvalidateRequests()
		...
		for initiator, _ in pairs(viewRequests) do
			for chip, _ in pairs(viewRequests[initiator]) do
				if ValidateRequest(initiator, chip) then
					count = count + 1
				elseif not viewRequests[initiator] then
					break -- If that validation removed the entire initiating player, stop trying to enumerate their requests
				end
			end
		end
		...
		if count == 0 then
			hook.Remove("Tick", "WireExpression2_InvalidateRequests")
		end
	end
	...
	function TOOL:Do_RightClick()
		...
		if IsValid(chip) and chip:GetClass() == "gmod_wire_expression2" then
			...
			elseif WireLib.CanTool(player, chip, "wire_expression2") then
				-- The player has prop protection perms on the chip
				self:Download(player, chip)
				player:SetAnimation(PLAYER_ATTACK1)
				...
				BetterChatPrint(chip.player, string.format("The %s '%s' just accessed your chip '%s' via prop protection", playerType, player:Nick(), chip.name))
			elseif chip.alwaysAllow and chip.alwaysAllow[player] then
				-- The player doesnt have prop protection perms, however the owner always allows for this chip (or they're invalid)
				self:Download(player, chip)
				player:SetAnimation(PLAYER_ATTACK1)
			...
			net.Start("WireExpression2_OpenEditor")
			net.Send(player)
		end
	end
	...
	function WireLib.Expression2Download(ply, targetEnt, wantedfiles, uploadandexit)
		...
		elseif not wantedfiles then
			...
			timer.Simple(60, function()
				-- make permissions timeout after 60 seconds
				if not IsValid(targetEnt) then
					return
				end
				if not targetEnt.DownloadAllowedPlayers then
					return
				end
				if not targetEnt.DownloadAllowedPlayers[ply] then
					return
				end
	...
	net.Receive("wire_expression2_download_wantedfiles", function(len, ply)
		...
		if not toent.DownloadAllowedPlayers or not toent.DownloadAllowedPlayers[ply] then
			return
		end
	...
	concommand.Add("wire_expression_forcehalt", function(player, command, args)
		...
		E2 = Entity(E2)
		if not IsValid(E2) or E2:GetClass() ~= "gmod_wire_expression2" then
			return
		end
		if canhas(player) then
			return
		end
		if E2.error then
			return
	...
	concommand.Add("wire_expression_requestcode", function(player, command, args)
		...
		elseif WireLib.CanTool(player, E2, "wire_expression2") then
			...
			BetterChatPrint(E2.player, string.format("The %s '%s' just accessed your chip '%s' via prop protection", playerType, player:Nick(), E2.name))
	...
	concommand.Add("wire_expression_reset", function(player, command, args)
		...
		if WireLib.CanTool(player, E2, "wire_expression2") then
			if E2.context.data.last or E2.first then
				return
			end
			E2:Reset()
...
elseif CLIENT then
	...
	local function next_queue()
		...
		while true do
			...
			if queue[1].timeStarted + 2 < CurTime() and not IsValid(Entity(queue[1].targetEnt)) then -- only clear it if more than 2 seconds has passed since the upload was requested (if the user has high ping)
				table.remove(queue, 1)
			else
				break
			end
		...
		timer.Simple(1, function()
			-- wait a while before doing anything so stuff doesn't lag
			if #queue == 0 then
	...
	upload_queue = function(first)
		...
		for i = 1, #datastr, 64000 do
			timer.Simple(delay, function()
				packet = packet + 1
				if timeStarted + 2 < CurTime() and not IsValid(Entity(targetEnt)) then -- only remove the E2 from the queue if more than 2 seconds has passed since the upload was requested (if the user has high ping)
					if exited then
						return
					else
						exited = true
	...
	function WireLib.Expression2Upload(targetEnt, code, filepath)
		if not targetEnt then
			targetEnt = LocalPlayer():GetEyeTrace().Entity or NULL
		end
		if isentity(targetEnt) then
			if not IsValid(targetEnt) then -- We don't know what entity its going to
				return
			end
		...
		if not code and not wire_expression2_editor then -- If the player leftclicks without opening the editor or cpanel (first spawn)
			return
		end
	...
	net.Receive("wire_expression2_download_wantedfiles_list", function(len)
		...
		function selectall:DoClick()
			...
			for i = 1, #checkboxes do
				if not checkboxes[i]:GetChecked() then -- checkboxes[i]:SetChecked( true )
					checkboxes[i]:Toggle()
				end
			end
		end
		...
		function selectnone:DoClick()
			selectedfiles = {}
			for i = 1, #checkboxes do
				if checkboxes[i]:GetChecked() then -- checkboxes[i]:SetChecked( false )
					checkboxes[i]:Toggle()
				end
			end
		end
		...
		function ok:DoClick()
			...
			for k, v in pairs(selectedfiles) do
				haschoice = true
				break
			end
	...
	function TOOL.BuildCPanel(panel)
		...
		panel:AddControl("MaterialGallery", {
			...
			Options = {
				["Modern"] = {
					wire_expression2_select = "Modern",
					Value = "Modern",
					Material = "beer/wiremod/gate_e2",
					wire_expression2_model = "models/beer/wiremod/gate_e2.mdl",
				},
				["Expression"] = {
					wire_expression2_select = "Expression",
					Value = "Expression",
					Material = "models/expression 2/exprssn",
					wire_expression2_model = "models/expression 2/cpu_expression.mdl",
				},
				["Microchip"] = {
					wire_expression2_select = "Microchip",
					Value = "Microchip",
					Material = "models/expression 2/mcrochp",
					wire_expression2_model = "models/expression 2/cpu_microchip.mdl",
				},
				["Interface"] = {
					wire_expression2_select = "Interface",
					Value = "Interface",
					Material = "models/expression 2/intrfce",
					wire_expression2_model = "models/expression 2/cpu_interface.mdl",
				},
				["Controller"] = {
					wire_expression2_select = "Controller",
					Value = "Controller",
					Material = "models/expression 2/cntrllr",
					wire_expression2_model = "models/expression 2/cpu_controller.mdl",
				},
				["Processor"] = {
					wire_expression2_select = "Processor",
					Value = "Processor",
					Material = "models/expression 2/prcssor",
					wire_expression2_model = "models/expression 2/cpu_processor.mdl",
				},
			},
		})

		if wire_expression2_editor == nil then
			initE2Editor()
		end
	...
	if CogTexture == surface.GetTextureID("texturemissing") then
		CogTexture = nil
	end
...
function TOOL:GetModel()
	...
	if scriptmodel and scriptmodel ~= "" and validModelCached(scriptmodel) then
