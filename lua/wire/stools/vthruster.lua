...
if CLIENT then
	...
	TOOL.Information = {
		...
		},
		{
			name = "left_1",
			stage = 1,
			text = "Set the Angle, hold Shift to lock to 45 degrees",
		},
	}
end
...
if SERVER then
	function TOOL:GetConVars()
		return self:GetClientNumber("force"),
			...
			self:GetClientNumber("mode"),
			self:GetClientNumber("angleinputs") ~= 0,
			self:GetClientNumber("lengthismul") ~= 0
	end
end
...
function TOOL.BuildCPanel(panel)
	...
	panel:AddControl("ListBox", {
		...
		Options = {
			...
			["#XYZ World"] = {
				wire_vthruster_mode = "1",
			},
			["#XY Local, Z World"] = {
				wire_vthruster_mode = "2",
			},
		},
	})
