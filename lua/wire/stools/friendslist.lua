...
if CLIENT then
	...
	language.Add(
		"wire_friendslist_save_on_entity_help",
		"When 'Save On Entity' is enabled, the SteamIDs entered into the user interface below will be saved on the entity and carried across in dupes. When disabled, any changes you make in this UI below will be immediately synced to the friendslist entity."
	)
...
local function netReadValues()
	...
	for i = 1, net.ReadUInt(11) do
		...
		if bytes > 32 * 2 ^ 11 then --[[ someone is writing more than they are supposed to be. ]]
			return t
		end
	end
	return t
...
if SERVER then
	...
	function TOOL:GetConVars()
		return self:GetClientNumber("save_on_entity") ~= 0, friends[self:GetOwner()] or {}, self:GetClientNumber("sync_with_steam") ~= 0, self:GetClientNumber("sync_with_cppi") ~= 0
	end
	...
	function TOOL:RightClick(trace)
		if not self:CheckHitOwnClass(trace) then
			return false
		end
	...
	local function loadFile()
		if not file.Exists("wire_friendslist.txt", "DATA") then
			return {}
		end
